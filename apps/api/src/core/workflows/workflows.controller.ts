...
export class WorkflowsController {
  ...
  async create(@Body() body: { name?: unknown; description?: unknown; definition?: unknown; teamId?: unknown }, @CurrentUser() user?: AuthPrincipal) {
    ...
    const row = await this.workflows.create({
      ...
      // BG3 team-scoping: persist the team + creator so the workflow can be
      // scoped to a team. The management gate stays WORKFLOW_MANAGE (admin);
      // team-scoped management becomes available once team roles exist.
      teamId: body.teamId === undefined ? undefined : String(body.teamId),
