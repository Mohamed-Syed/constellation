...
describe("DelegationService — portal delegation view (Phase 4.0 backlog #1)", () => {
  it("listDelegations returns only tasks that HAVE children, each with the tree + budget flow-down", async () => {
    const db = makeDb();
    // p1 has children → should appear; solo (no children) → should not.
    db.rows.set("p1", row("p1", { status: "running", actorId: "boss" }));
    db.rows.set("c1", row("c1", { parentTaskId: "p1", status: "completed", totalTokens: 40, costUSD: 0.004 }));
    db.rows.set("solo", row("solo", { status: "completed" }));
    // The roots query uses `where.parentTaskId === null AND children.some` —
    // emulate that: return rows whose parentTaskId is null and that have kids.
    const realFindMany = db.agentTask.findMany;
    (db.agentTask as unknown as { findMany: unknown }).findMany = vi.fn(
      async ({ where, orderBy }: { where?: Record<string, unknown>; orderBy?: { createdAt?: string } }) => {
        if (where && (where as Record<string, unknown>).parentTaskId === null && (where as Record<string, unknown>).children) {
          const withKids = Array.from(db.rows.values()).filter((r) => {
            if (r.parentTaskId !== null) return false;
            return Array.from(db.rows.values()).some((x) => x.parentTaskId === r.id);
          });
          if (orderBy?.createdAt === "desc") withKids.sort((a, b) => String(b.createdAt).localeCompare(String(a.createdAt)));
          return withKids;
        }
        return realFindMany({ where, orderBy });
      },
    );
    const { svc } = makeSvc({ db });
    const delegations = await svc.listDelegations(10);
    // solo has no children → excluded.
    expect(delegations.length).toBe(1);
    expect(delegations[0]?.id).toBe("p1");
    expect(delegations[0]?.childCount).toBe(1);
    expect(delegations[0]?.childrenTotalTokens).toBe(40);
    expect(delegations[0]?.childrenCostUSD).toBeCloseTo(0.004, 5);
    expect(delegations[0]?.teamId).toBeNull();
  });

  it("listDelegations returns [] on no-DB and is capped", async () => {
    const { svc } = makeSvc({ db: undefined });
    expect(await svc.listDelegations(10)).toEqual([]);
  });
});
