...
@Module({
  ...
  providers: [
    ...
    // with fallback to providers[0]. Add another provider by registering it
    // here and appending it to the factory array — callers never change.
    //
    // ORDER MATTERS: providers[0] is the operator's default AND the failure
    // fallback. 2026-08-05 (operator decision): DeepSeek is first — the
    // engine's no-model tasks and cloud-failure fallbacks land on DeepSeek,
    // not Ollama (Ollama 7b-on-CPU was timing out in live use; the operator
    // runs the engine on DeepSeek and keeps Ollama for local/embed only).
    OllamaModelProvider,
  ...
  exports: [TaskService, TaskQueueService, ScheduledTaskService, SchedulerEngineService, SupervisorService, EngineAlertService, EngineAvailabilityService, ModelRouterService, DelegationService, McpClientService],
})
