...
export class DeepSeekModelProvider implements ModelProvider {
  ...
  async chat(messages: ChatMessage[], model?: string): Promise<ChatResponse> {
    const resolvedModel = this.normalizeModel(model) ?? this.defaultModel;
  ...
  /**
   * The DeepSeek REST API accepts BARE model ids only (deepseek-v4-flash).
   * The portal & router may carry a "deepseek:"-prefixed composite hint
   * (e.g. the compare UI sends "deepseek:deepseek-v4-flash"); strip that
   * prefix before the model id reaches the API, or DeepSeek rejects it as an
   * unknown model (terminal 4xx → the worker spins and the task eventually
   * fails "Reached max steps without completing"). Returns the bare id.
   */
