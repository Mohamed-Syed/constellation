...
import { asMessage } from "./error-utils.js";
...
export class EngineController {
  ...
  /**
   * Portal delegation view (Phase 4.0 backlog #1) — every crew root (a task
   * with children), each with its full tree + descendant budget flow-down.
   * Scope: same visibility as the engine task list (admin sees all; others see
   * personal + their teams').
   */
  @Get("delegations")
  async delegations(@CurrentUser() user: AuthPrincipal) {
    const isAdmin = user?.permissions?.includes("platform:admin") ?? false;
    let roots: Awaited<ReturnType<DelegationService["listDelegations"]>> = [];
    if (isAdmin) {
      roots = await this.delegation.listDelegations(50);
    } else {
      // Restrict to roots the caller owns or that belong to their teams. We
      // filter the returned trees by actorId / teamId.
      const myTeams = (await this.teams.listForUser(user?.id ?? null)).map((t) => t.id);
      const all = await this.delegation.listDelegations(100);
      roots = all.filter((r) => {
        if (r.actorId && r.actorId === user?.id) return true;
        return typeof r.teamId === "string" && myTeams.includes(r.teamId);
      });
    }
  ...
  /**
   * Phase 4.0 backlog #6 — CROSS-INSTANCE MESH TASK ROUTING (receiver side).
   * A trusted peer (via the shared mesh route key) enqueues a task on THIS
   * instance. Gated by the `x-mesh-route-key` header matching env
   * `MESH_ROUTE_API_KEY` — NOT a user token, because the caller is another
   * Constellation instance. When the key is unset this endpoint is inert (403),
   * so routing is opt-in and never opens a hole by default.
   */
  @Post("mesh/forward")
  @Public()
  async meshForward(@Req() req: { headers: Record<string, string | string[] | undefined> }, @Body() body: Record<string, unknown>) {
    const key = this.config?.get("MESH_ROUTE_API_KEY");
    const supplied = extractHeader(req.headers, "x-mesh-route-key");
    if (!key || !supplied || supplied !== key) {
      throw new ForbiddenException("Mesh routing is not configured or the route key is invalid.");
    }
    ...
    const task = await this.tasks.create(dto, undefined);
    try {
      await this.queue.enqueue(task.id);
    } catch (err) {
      throw new ServiceUnavailableException(`Engine queue unavailable while forwarding: ${asMessage(err)}`);
    }
    return { taskId: task.id, status: task.status, forwarded: true };
  }
...
/** Extract a case-insensitive single header value from a raw header record. */
function extractHeader(headers: Record<string, string | string[] | undefined>, name: string): string | null {
