...
export class TaskService {
  ...
  async findAll(opts: { teamId?: string; actorId?: string | null; teamIds?: string[] } = {}) {
    ...
    return db.agentTask.findMany({
      ...
      select: {
        ...
        inputTokens: true, outputTokens: true, totalTokens: true, costUSD: true,
  ...
  /**
   * Failed (dead-lettered) task rows — the durable DLQ list, distinct from
   * BullMQ's failed-JOB view. Newest-first by default (the engine view's
   * display order); `order: "asc"` serves the AI Controller's
   * re-enqueue-deadletters action, which must retry the OLDEST failures first
   * so a large backlog doesn't starve its tail. Capped at `limit`.
   */
  async findAllFailed(limit = 100, order: "asc" | "desc" = "desc") {
    const db = this.prisma.db;
    if (!db) return [];
    return db.agentTask.findMany({
      where: { status: "failed" },
  ...
  /**
   * AI Controller (Phase 5.0) — re-enqueue a FAILED (dead-lettered) task:
   * flip it back to `queued` so the worker picks it up again. Unlike
   * `markResumed` (supervisor resume-once semantics), this is the explicit
   * "try the dead letter again" path used by the `re-enqueue-deadletters`
   * controller action.
   *
   * The update is GATED on the row still being `failed` (updateMany + count):
   * a concurrent re-enqueue or a worker that already completed the task can
   * never have this call resurrect it. Returns `false` — without ever
   * throwing — when the DB is absent, the row is not `failed`, or the write
   * fails.
   */
