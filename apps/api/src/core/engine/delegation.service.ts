...
export interface DelegationTreeNode {
  ...
  model: string | null;
  actorId: string | null;
  /** Team the crew belongs to (null = personal). Used for delegation-view RBAC. */
  teamId: string | null;
  stepCount: number;
...
export class DelegationService {
  ...
  async tree(parentId: string): Promise<DelegationTreeNode> {
    ...
    if (!db) {
      this.warnNoDbOnce();
      return { id: parentId, title: "", status: "unknown", provider: null, model: null, actorId: null, teamId: null, stepCount: 0, totalTokens: null, costUSD: null, createdAt: "", completedAt: null, children: [] };
    }
  ...
  /**
   * Portal delegation view (Phase 4.0 backlog #1) — list every task that has
   * child tasks (i.e. every delegation root / crew parent), each with its
   * fully-materialized delegation tree + descendant budget flow-down. Capped
   * to the most recent `limit` roots. Returns [] on no-DB — never throws.
   */
  async listDelegations(limit = 50): Promise<DelegationTreeNode[]> {
    const db = this.prisma.db;
    if (!db) {
      this.warnNoDbOnce();
      return [];
    }
    // Find tasks that ARE a parent (parentTaskId null = top of a tree, and
    // they must have at least one child). Prisma exposes the child count via
    // `_count` without loading the rows.
    const roots = await db.agentTask.findMany({
      where: { parentTaskId: null, children: { some: {} } },
      orderBy: { createdAt: "desc" },
      take: Math.max(1, Math.min(limit, 100)),
      select: { id: true },
    });
    const out: DelegationTreeNode[] = [];
    for (const root of roots) {
      if (!root?.id) continue;
      const tree = await this.tree(root.id);
      // Skip trees that are somehow empty/missing (a race where the root was
      // detached between the two queries).
      if (tree.id === root.id || tree.status !== "unknown") out.push(tree);
    }
    return out;
  }
