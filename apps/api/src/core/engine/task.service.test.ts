...
describe("TaskService — requeue (AI Controller re-enqueue-deadletters)", () => {
  it("flips a failed task back to queued, gated on the row still being failed", async () => {
    const { svc, db } = serviceWith(makeDb());
    db.agentTask.updateMany.mockResolvedValue({ count: 1 });

    const ok = await svc.requeue("dl1");

    expect(ok).toBe(true);
    expect(db.agentTask.updateMany).toHaveBeenCalledWith({
      where: { id: "dl1", status: "failed" },
      data: { status: "queued" },
    });
  });

  it("returns false when the row is no longer failed (count 0 — never resurrects)", async () => {
    const { svc, db } = serviceWith(makeDb());
    // The worker already completed the task mid-loop: the gated update matched
    // nothing, so the caller must not count it as re-enqueued.
    db.agentTask.updateMany.mockResolvedValue({ count: 0 });
  ...
  it("returns false when the write rejects (P2025 / DB death) — never throws", async () => {
    const { svc, db } = serviceWith(makeDb());
    db.agentTask.updateMany.mockRejectedValue(new Error("P2025 row not found"));

    await expect(svc.requeue("dl1")).resolves.toBe(false);
  });

  it("returns false without a database (never throws)", async () => {
    const { svc } = serviceWith(undefined);
    await expect(svc.requeue("dl1")).resolves.toBe(false);
  });
});
