...
export class TaskQueueService implements OnModuleInit, OnModuleDestroy {
   ...
   *
   * Never throws: if Redis dies AFTER boot (the availability probe only runs
   * at boot) the counter reads reject — that is caught and reported as
   * `enabled:false` with the reason, so consumers (the AI Controller's status
   * snapshot) see a genuinely unhealthy queue instead of a silent null.
   */
  async getHealth() {
    ...
    try {
      ...
      return {
        enabled: false,
        reason: `queue unreachable: ${err instanceof Error ? err.message : String(err)}`,
      };
    }
  }
}
