...
/**
 * ModelRouterService tests. The only external seam is global `fetch`
 * (provider-agnostic by design), so each test stubs `globalThis.fetch` with
 * `vi.stubGlobal` and restores it after — no test opens a socket.
 *
 * Contracts under test:
 *  1. `chat()` POSTs { model, messages, stream:false } to <base>/api/chat and
 *     normalizes the Ollama response into a ChatResponse.
...
describe("ModelRouterService — chat()", () => {
  it("POSTs to <base>/api/chat and parses a successful response", async () => {
    ...
    const svc = new ModelRouterService(makeConfig());
  ...
  it("honours OLLAMA_BASE_URL from config", async () => {
    ...
    const svc = new ModelRouterService(makeConfig({ OLLAMA_BASE_URL: "http://ollama:11434" }));
  ...
  it("aborts the request when the model timeout elapses", async () => {
    ...
    const svc = new ModelRouterService(makeConfig({ MODEL_TIMEOUT_MS: "10" }));
...
describe("ModelRouterService — health()", () => {
