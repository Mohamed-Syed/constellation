...
export class ModelRouterService {
   ...
   *  - No model → providers[0] (the operator's default — historically
   *    Ollama; since 2026-08-05 DeepSeek — see engine.module.ts ORDER MATTERS).
   ...
   *  - No match → FALL BACK to providers[0] with ITS default model:
   ...
   *    providers[0]'s own DEFAULT_MODEL (same rule as the chat-failure
   *    fallback).
  ...
  private selectProvider(requestedModel?: string): { provider: ModelProvider; model: string | undefined } {
    ...
    this.logger.warn(
      `Model router: no provider can handle "${requestedModel}" — falling back to ${fallback.name} with its default model`,
    );
  ...
  private async runChat(provider: ModelProvider, messages: ChatMessage[], resolvedModel: string | undefined): Promise<ChatResponse> {
    try {
      ...
      // hiccup, bad key, quota) falls back to providers[0] instead of failing
      // the task. The default provider (providers[0]) failing is the task
      // failing.
      if (provider === this.providers[0]) throw err;
      const message = err instanceof Error ? err.message : String(err);
      this.logger.warn(`${provider.name} chat failed, falling back to ${this.providers[0]?.name ?? "default"}: ${message}`);
      const fallback = this.providers[0];
