...
describe("NotificationService — record", () => {
  ...
  it("persists a recipientId when a specific user is targeted", async () => {
    const db = makeDb();
    const svc = svcWith(db);
    await svc.record("report.generated", "info", "Weekly report", "pdf ready", "report", "r1", "user-42");
    const data = (db.notification.create.mock.calls[0]?.[0] as { data: Record<string, unknown> }).data;
    expect(data.recipientId).toBe("user-42");
  });
...
describe("NotificationService — list / unreadCount", () => {
  it("returns items newest-first plus the total unread count", async () => {
    ...
    const svc = svcWith(db);
    const result = await svc.list(null, 25);
    ...
    expect(db.notification.count).toHaveBeenCalledWith({
      where: { recipientId: null, read: false },
    });
  ...
  it("applies kind and unread filters and clamps the limit to [1, 100]", async () => {
    const db = makeDb();
    const svc = svcWith(db);
    await svc.list(null, 500, "engine.task.failed", true);
    expect(db.notification.findMany).toHaveBeenCalledWith({
      where: { recipientId: null, kind: "engine.task.failed", read: false },
      orderBy: { createdAt: "desc" },
      take: 100,
    });
    await svc.list(null, 0);
    expect(db.notification.findMany).toHaveBeenCalledWith(
      expect.objectContaining({ take: 1 }),
    );
  });

  it("scopes list to a caller: global + their targeted notifications", async () => {
    const db = makeDb();
    const svc = svcWith(db);
    await svc.list("user-1", 50);
    expect(db.notification.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { OR: [{ recipientId: null }, { recipientId: "user-1" }] },
      }),
    );
  });
  ...
  it("degrades to an empty feed without a database", async () => {
    const svc = new NotificationService({ db: undefined } as unknown as PrismaService);
    await expect(svc.list(null)).resolves.toEqual({ items: [], unreadCount: 0 });
  ...
  it("returns 0 when the count query fails", async () => {
    ...
    const svc = svcWith(db);
    await expect(svc.unreadCount("u1")).resolves.toBe(0);
  });
});
