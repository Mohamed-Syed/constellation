...
import { BarChart3, CheckCircle2, Clock, Coins, Copy, FileText, Loader2, Play, XCircle } from "lucide-react";
...
export function CompareView() {
  ...
  const run = async () => {
    ...
    const entries: CompareResult[] = selected.map((key) => {
      ...
      return { model: m ? `${m.provider} · ${m.model}` : key, taskId: null, detail: null, error: null, running: true };
    });
    setResults(entries);
    for (let i = 0; i < entries.length; i++) {
      ...
      if (!submit.ok) {
        setResults((prev) =>
          prev.map((r) => (r.model === entry.model ? { ...r, running: false, error: submit.message } : r)),
        );
        continue;
      }
      const detail = await pollTask(submit.task.id);
      setResults((prev) =>
        prev.map((r) => (r.model === entry.model ? { ...r, running: false, taskId: submit.task.id, detail } : r)),
      );
    }
    setRunning(false);
  };
        ...
        <div className="flex items-center gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
              Run the same prompt against 2+ models side-by-side — latency, tokens, cost, output.
            </p>
          </div>
        </div>
      </Reveal>
                ...
                <Reveal key={r.model} delay={0.05}>
                  <Card className={r.detail?.status === "failed" ? "border-rose-300/40 dark:border-rose-500/20" : ""}>
                    <CardHeader>
                      ...
                      <CardDescription>
                        {r.detail
                          ? `${r.detail.status} · ${formatLatency(r.detail)} · ${r.detail?.totalTokens ?? 0} tokens · ${formatCost(r.detail?.costUSD ?? null)}`
                          : "pending"}
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
