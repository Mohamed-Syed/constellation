...
const CORE_PLATFORM: FlatNavItem[] = [
  ...
  {
    // The in-app Knowledge base — end-user documentation for the whole
    // platform (this is where every operator learns the application).
    id: "core-docs",
  ...
  {
    // Phase 4.0 backlog #1 — portal-wide delegation view: every agent crew
    // (parent → children) with budget flow-down, beyond the per-task dialog.
    id: "core-delegations",
    label: "Delegations",
    href: "/delegations",
    icon: "GitBranch",
  ...
  {
    // Phase 4.0 4.6 — federated agent mesh: the fleet topology (register/probe/
    // remove peers). Admin-gated — hidden unless the caller can manage the mesh.
    id: "core-mesh",
    label: "Mesh",
    href: "/mesh",
    icon: "Share2",
    order: 33,
    requiresAnyPermission: ["core:mesh:manage"],
  },
  {
    // Phase 5.0 — Agentic AI Controller: the platform's live stability score +
    // findings + whitelisted recovery actions. Admin-gated like the audit
    // surface (core:audit:read — platform:admin implies it).
    id: "core-ai-controller",
    label: "AI Controller",
    href: "/ai-controller",
    icon: "Radar",
    order: 34,
    requiresAnyPermission: ["core:audit:read"],
  },
  {
...
/** Permissions that unlock the "Admin" nav entry (either satisfies it). */
...
/**
 * Role-aware nav (generalized for the Brain round): hide any
