 ...
 *
 * CONTRACT (docs/BRAIN.md §5, owned by the core API in `apps/api/src/core/memory`):
 ...
 *
 * The engine is a Graphify sidecar that may not be running, and the API routes
...
function classify<T>(status: number): BrainResult<T> {
  ...
  if (status === 404 || status === 501 || status === 503) {
    // 404 = the /api/brain routes aren't mounted yet (the API lane) or there's no
