...
/**
 * Shared bearer-token header builder for API calls. The single source for
 * `Authorization: Bearer …` (+ optional JSON content type) — previously
 * copy-pasted across lib/mesh.ts, lib/teams.ts and a dozen more modules
 * (the cleanup pass migrated the first two; the rest are a follow-up).
 */
export function authHeaders(token: string | null | undefined, json = false): Record<string, string> {
...
export async function getPlugins(token?: string): Promise<PluginSummary[]> {
  try {
    const res = await fetch(`${API_BASE}/plugins`, { cache: "no-store", headers: authHeaders(token ?? null) });
    if (!res.ok) return [];
