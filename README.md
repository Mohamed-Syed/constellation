...
- Live `/engine` task console, `/health` dashboard, `/compare` (multi-model A/B)
...
| 📔 **Development journal** | `docs/MASTER_PLAN.md`, `docs/HANDOFF.md`, `docs/ORCHESTRATOR.md`, `docs/SUPER_SESSION_SUMMARY.md`, and `artifacts/*/live-proof-evidence.md` — the honest, round-by-round record of how this was built, including every bug found and every live proof. See the [note on how this was built](#how-this-was-built). |
...
- ✅ Every major feature has a live-proof record under [`artifacts/`](artifacts/)
- 💵 Built and proven entirely **local, ≈$0** — no cloud provisioned

### How this was built

Constellation was built **with AI assistance** — a small team of AI coding agents (a lead orchestrator plus focused implementers, working in disjoint lanes under a maker/checker discipline) drove the architecture, the engine, and the hardening, under a human product owner whose day job is network engineering. The `docs/` development journal and `artifacts/` evidence files are kept deliberately transparent: they record every round, every bug found in live testing (and there were many the green tests missed), and exactly what is and isn't verified. That transparency is a feature — it's the honest story of how a real platform gets built.
