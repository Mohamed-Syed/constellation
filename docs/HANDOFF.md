...
>
> **Last updated:** 2026-08-04 (Phase 2.0 OTel tracing LIVE-PROVEN — additive tracer, Tempo backend, spans for HTTP/engine/model/tool; plus the three post-v0.6 rounds backfilled) · **Updated by:** Polaris
...
- ✅ **2.3 Prometheus metrics** — `/api/metrics` DONE (`ac2cf11`); remaining: pre-built Grafana dashboard JSON.
- ✅ **2.5 `constellation` CLI ops** DONE (`e8fe871`).
- ⏳ **Remaining:** 2.4 portal `/health` dashboard (P1), 2.6 real SSO round-trip (P1 — Keycloak+Caddy are booted in the federation overlay; the OIDC seam + a real login round-trip are unproven end-to-end), 2.7 plugin sandboxing (P1), 2.8 worker as separate process (P2).
