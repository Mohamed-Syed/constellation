...
>
> **Last updated:** 2026-08-03 (Engine v0.5 Reliability round COMPLETE — dead-letter + supervisor + alerts LIVE-PROVEN, 505 tests) · **Updated by:** Polaris
...
- **🤖 ENGINE v0.5 — DEEPER 24/7 RELIABILITY ✅ COMPLETE (Polaris orchestrating via delegate_task, 2026-08-03). DONE, gate-verified, LIVE-PROVEN, committed — see MASTER_PLAN §9.** Structured **dead-letter** trail, a **supervisor** for stuck tasks, and **event-based alerting**. `FailureClassification` (terminal/transient_exhausted/stalled/rejected), `TaskService.findAllFailed` + `getFailedCount`, `GET /api/engine/deadletters`; `SupervisorService` (poll loop, race-guard vs active jobs, resume-once, re-stale → `stalled` dead letter, no infinite spin, `ENGINE_SUPERVISOR_INTERVAL_MS`/`ENGINE_STALE_TASK_MS`); `EngineAlertService` (emits `engine.task.failed/stale/recovered` on the EventBus + in-memory trail via `GET /api/engine/alerts` + health). **PROVEN LIVE:** a stale task was flagged (`engine.task.stale`, 122239ms) and recovered (`engine.task.recovered`) then completed on Ollama (no double-run); a re-stale task became `failed/failureClassification=stalled`. **Tests: 481 → 505** (api 364 → **388**). Fixed a boot DI bug (EngineAlertService optional cap param) the offline tests missed.
