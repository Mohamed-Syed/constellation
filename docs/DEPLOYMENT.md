...
The **in-app Knowledge base** (`/docs` — 32 end-user articles across 9 sections) ships inside the portal build; nothing extra to run, no extra env vars.
...
| `DEFAULT_MODEL` | `deepseek-v4-flash` | Default model id (engine default since the 2026-08-05 operator decision; previously the local Ollama model) |
...
| Variable | Default | Purpose |
|---|---|---|
| `MESH_PROBE_INTERVAL_MS` | `60000` | How often the mesh prober re-checks every registered peer's `/api/health` |
| `MESH_PROBE_TIMEOUT_MS` | `5000` | Per-peer probe timeout (ms); a non-2xx/timeout flips the peer to `down` |

### Agentic AI Controller (Phase 5.0)

| Variable | Default | Purpose |
|---|---|---|
| `CONTROLLER_WATCH_ENABLED` | `on` | The autonomous watch loop. `off` disables it (the manual `GET /api/ai-controller/status` + `POST /api/ai-controller/act` surfaces stay available). |
| `CONTROLLER_WATCH_INTERVAL_MS` | `30000` | Cadence of the autonomous loop (min 5000). Each tick scores the platform, notifies on score/label transitions, and runs due safe recovery actions itself (`reprobe-mesh`, `re-enqueue-deadletters`, `flush-stale`) — per-action cooldowns prevent churn; every autonomous action is audited as an `ai-controller.autonomous` notification. |
