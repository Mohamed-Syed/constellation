...
| **Team spaces portal (3.7, slice 2)** | Polaris (solo) | /teams page + TeamsView (create team → owner, my-teams role badges, members by email + role select, remove w/ owner protection), lib/teams.ts, core-teams nav; TaskService list projection + usage/cost; 1 live-found path bug (double /api prefix) + unhydrated-dev-server gotcha | `7216f45` | `docs-commit-below` | 554 → **554** (web gates only) | ✅ LIVE: viewer /me teams + team task via ?teamId= + 403 member mgmt; browser OWNER badge + member rows + ADD MEMBER box verified |
| **SMTP email channel (3.5 remainder)** | Polaris (solo) | Zero-dep net/tls SMTP client (smtp-send.ts), channel type smtp (recipient/from, env relay), injectable sender, honest no-SMTP_HOST degrade, .env.example block; 2 real bugs found live (multi-line EHLO folding, 3xx DATA go-ahead) | `359ece8` | `docs-commit-below` | 554 → **559** (+5) | ✅ LIVE vs local SMTP stub: test mail + real engine.task.failed mail (subject/body recorded literally) |
| **MCP server (4.3)** | Polaris (solo) | POST /api/mcp JSON-RPC 2.0 (initialize/tools/list/tools/call/ping/resources), 4 constellation tools (list_tasks/run_task/engine_health/list_schedules), JWT-guarded, error codes | `609da8a` | `docs-commit-below` | 559 → **568** (+9) | ✅ LIVE client exchange: handshake + a REAL task completed via MCP on Ollama (mcp-ok, 424 tokens, $0) + isError/401 |
| **Compliance export (4.7)** | Polaris (solo) | GET /api/audit/export (CSV, actor/action filters, cap 1000), pure auditToCsv (metadata JSON, quoting tested), portal Export CSV button on the Audit tab | `518a5b3` | `docs-commit-below` | 568 → **571** (+3) | ✅ LIVE: 200/2415-byte real CSV + workflow-filtered + 401; browser button toast |
| **Brain UX + docs-mode RAG (4.2 foundation)** | Polaris (solo) | Brain page: adaptive force-layout iterations, content-keyed layout memo (poll-jank fix), label anti-collision + zoom gating, degraded banner. RAG: GRAPHIFY_MODE=docs default, graphify-out bind mount (named-volume root cause), 7b model, Dockerfile [mcp,ollama]; 2 real failures fixed live (openai package missing, 1.5b hollow JSON) | `70e81a1` | `docs-commit-below` | api unchanged; web gates clean | ✅ /brain NOT-BUILT UI browser-verified; extraction live (258 code + 11 docs); graph materialization running at close (7b semantic pass) |

**Final tree at `e54cae6`:** clean (only untracked `Prompt to Clau_Partner.txt`).
