...
| 3.5 | ✅ **Notification center** — **SHIPPED 2026-08-04 (round 3.4, §9):** durable in-portal event feed (engine alerts + scheduler events), REST API, sidebar unread badge, admin audit-log tab. Remaining from the original vision (documented follow-ups): notification **channels** (in-portal toast, email via SMTP, webhook, Slack/Discord/Teams) + user-configurable per task. | "Runs while you sleep" — but you need to know when it's done. Every enterprise platform has this. | Grafana alerting, PagerDuty, every CI system | P1 |
