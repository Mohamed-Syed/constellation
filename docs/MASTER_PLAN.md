...
> **Status:** Platform layer strong + **the agentic engine is real, live-proven, and now self-recovering** through **Engine v0.5** (git `ec88534`, local only — never pushed). Durable BullMQ task runtime, ReAct loop with checkpoint/resume (kill-restart proven live), **human-in-the-loop approval gate**, per-task token budget cap, `/engine` portal, real model providers (OpenRouter + routing/fallback + cost-aware budget), a **scheduler** (cron + event auto-enqueue, live-proven), and **Engine v0.5 — DEEPER 24/7 RELIABILITY**: structured **dead-letter** trail (terminal/transient_exhausted/stalled/rejected classifications, `GET /api/engine/deadletters`), a **supervisor** that detects stuck `running` tasks and recovers them (resume-once, race-guarded; re-stale -> `stalled` dead letter, no infinite spin), and **event-based alerting** (`engine.task.failed/stale/recovered`, `GET /api/engine/alerts`). **PROVEN LIVE:** a stale task was flagged + recovered and completed; a re-stale task became a `stalled` dead letter; deadletters/alerts/health all report honestly. **505 tests green** (api 388, browser-use 47, graphify 40, sdk 21, cli 9). See §9. **Next: platform breadth** (migrations history, plugin sandboxing, viewer seed, httpOnly-cookie). VPS deferred (prove locally first).
> **Relationship to Looper:** SEPARATE project. Looper (`../loop-engineering`) is untouched.
> **Last updated:** 2026-08-03 (Polaris — Engine v0.5 Reliability round: dead-letter + supervisor + alerts, live-proven, 505 tests)
