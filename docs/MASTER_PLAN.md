...
| # | Feature | Why it matters | Competitor benchmark | Priority |
|---|---------|---------------|---------------------|----------|
| 4.1 | ✅ **Agent-to-agent delegation (crews)** — **SHIPPED 2026-08-05 (`b77e4ff`, §9):** DURABLE delegation — `AgentTask.parentTaskId` + DelegationService (spawnChild/childrenOf/tree/waitForChildren), REST delegate endpoints (owner/admin/team RBAC → 403), MCP `constellation.delegate_task` (MCP clients orchestrate crews), portal Delegation tree + Delegate form in the task detail dialog. LIVE: parent + 2 REST children + 1 MCP child all completed on Ollama; browser tree vision-verified. Remaining: budget flow-down accounting per sub-agent, delegation-tree view in the portal beyond one task's dialog, sub-agent result merging into the orchestrator's context. | Multi-agent collaboration is the frontier — CrewAI (52K stars) does this but without a durable engine. LangGraph DeepAgents does planner+subagent. Constellation would do it DURABLY with a real engine underneath. | CrewAI (52K GitHub stars), LangGraph DeepAgents, Microsoft AutoGen | P0 |
| 4.2 | ✅ **Persistent agent memory (RAG + Graphify)** — **FOUNDATION SHIPPED 2026-08-05 (`70e81a1`):** the docs-mode brain build (markdown over docs/ + brain/ via local Ollama embeddings) is the default; the built graph is now visible to the api (bind-mounted graphify-out); nomic-embed-text pulled; agents remember via graph.ingest → rebuild → node-appears. Remaining: the pgvector/Chroma semantic-retrieval layer (the graph is the memory; a vector store is the follow-up). | "What did I do last time?" — without persistent memory, every agent is amnesiac. This is the single biggest differentiator for a 24/7 agent platform. | CrewAI's memory system, LangChain's memory, MemGPT, Letta | P0 |
| 4.3 | ✅ **MCP (Model Context Protocol) server** — **SHIPPED 2026-08-05 (`609da8a`, §9):** `POST /api/mcp` (JWT-guarded), JSON-RPC 2.0 request/response (initialize / tools/list / tools/call / ping / resources/list), tools list_tasks / run_task / engine_health / list_schedules; a REAL task completed via MCP on Ollama in the live proof. Remaining: the MCP CLIENT side (agents calling external MCP servers as tools) + more tool surface. | MCP is becoming the USB-C of AI tools. Anthropic's standard, now adopted by OpenAI, Google, and the open-source ecosystem. Being an MCP server AND client makes Constellation the universal agent hub. | Anthropic MCP, OpenAI's adoption, the growing MCP ecosystem | P0 |
...
| 4.5 | ✅ **Autonomous incident response** — **PRIMITIVE SHIPPED 2026-08-05 (`ee5b304`, §9):** event-triggered workflows — a workflow on `engine.task.failed` auto-remediates failures (proven live: a real doomed task fired the remediation workflow). Remaining: Grafana/Prometheus alert ingestion as a trigger source + diagnostic tooling. | This is the "PagerDuty replacement" vision. AI agents as first-responders, humans as reviewers. Every SRE team wants this. | PagerDuty + AI, Datadog AI agent, but none self-hosted | P2 |
...
| 4.7 | ✅ **Audit + compliance reports** — **SHIPPED 2026-08-05 (`518a5b3`, §9):** `GET /api/audit/export` — the audit log as a downloadable CSV (actor/action filters, RFC-4180) + portal Export button. Remaining: PDF generation + scheduled report delivery. | Every enterprise buyer will ask for this. The immutable audit trail is already built — make it reportable. | ServiceNow GRC, Vanta, Drata | P2 |
