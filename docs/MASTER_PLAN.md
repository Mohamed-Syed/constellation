...
> **Status:** Platform layer strong + **the agentic engine is real, live-proven, self-recovering, and the platform is hardware/secrecy-hardened** through **v0.6** (git `070fb2d`), plus **Phase 2.0 production foundation**: committed Prisma migrations + Prometheus `/api/metrics` (`ac2cf11`), `constellation` ops CLI (`e8fe871`), the portal design-language rebuild (`b2d296b`→`635ec9d`), **OpenTelemetry tracing + Tempo** (`8d29e3f`), and the **portal `/health` engine dashboard** (`0647666`) — all local only, never pushed. Durable BullMQ task runtime, ReAct loop with checkpoint/resume (kill-restart proven live), **human-in-the-loop approval gate**, per-task token budget cap, `/engine` portal, real model providers (OpenRouter + routing/fallback + cost-aware budget), a **scheduler** (cron + event auto-enqueue), **Engine v0.5 24/7 reliability** (dead-letter trail + stuck-task supervisor + event alerting), and **Platform hardening v0.6** (a `viewer` non-admin user seed so the RBAC 403 path is live-testable; per-plugin Postgres schema bootstrap for C8; **httpOnly-cookie token auth** closing the localStorage XSS caveat). **547 tests green** (api 423, browser-use 47, graphify 40, sdk 21, cli 16). See §9. **Remaining (before publish):** plugin sandboxing (real enforcement), more capability plugins, pre-built Grafana dashboard JSON (the /health page + /api/metrics are live; the Grafana dashboard file is not). The real Windows username has been sanitized from tracked docs/scripts to a `<user>` placeholder (publish-readiness ✓). VPS deploy deferred (provider + budget = user decision).
> **Relationship to Looper:** SEPARATE project. Looper (`../loop-engineering`) is untouched.
> **Last updated:** 2026-08-04 (Polaris — Phase 2.0 2.4 portal /health dashboard LIVE-PROVEN; OTel round + three post-v0.6 rounds in §9)
...
| 3.4 | ✅ **Plugin scaffolding + hot-reload** — **SHIPPED (decision recorded 2026-08-05):** `constellation generate plugin my-plugin` (CLI, since round 1) creates the fully-typed starter (manifest, tools, tests); the marketplace's loader `reload()` hot-reloads installed plugins without a restart (proven live in round 3.2). | DX: 1 command, 1 minute to a working plugin. This is what VSCode extensions get right. | VSCode `yo code`, Backstage `@backstage/create-app`, Port's Ocean framework | P1 |
| 3.5 | ✅ **Notification center** — **SHIPPED 2026-08-04 (round 3.4, §9):** durable in-portal event feed (engine alerts + scheduler events), REST API, sidebar unread badge, admin audit-log tab. **Channels SHIPPED 2026-08-05 (`88944b7`):** webhook channels (generic/Slack/Discord/Teams envelopes, per-kind filters, Test) + engine.task.completed/paused events. **Email SHIPPED 2026-08-05 (`359ece8`):** SMTP channel (zero-dep net/tls client, env-configured relay). Remaining: per-task user-configurable notification. | "Runs while you sleep" — but you need to know when it's done. Every enterprise platform has this. | Grafana alerting, PagerDuty, every CI system | P1 |
| 3.6 | ✅ **Multi-model compare / A/B** — **SHIPPED 2026-08-05 (`f2afda4`, §9):** same prompt on 2+ models (Ollama local, DeepSeek/OpenRouter cloud) side-by-side in the portal — latency, tokens, cost (usage/cost now persisted per task). Quality scoring is a follow-up. | Model selection is a real operational decision — this makes it measurable. | OpenRouter's own compare view; LangSmith's evaluation | P2 |
| 3.7 | ✅ **Team spaces / multi-tenancy** — **COMPLETE 2026-08-05 (`418c0d8` API + `7216f45` portal, §9):** `Organization → Team → User` + TeamMember roles (owner/admin/member), inherited RBAC (member management owner/admin-only), `/api/teams` REST, `/me` carries teams, `AgentTask.teamId` scoping (submit-into-team requires membership; non-admins see personal + their teams' tasks), portal /teams page (create team, members by email, role select) — browser-proven. Follow-ups: team-scoped schedules/workflows/plugins, org roles, invitations. Team A can only see/edit their own tasks — admin sees everything. | The #1 request from any org with >5 users. | Every enterprise platform (Backstage RBAC, Port teams, GitHub orgs) | P2 |
...
| # | Feature | Why it matters | Competitor benchmark | Priority |
|---|---------|---------------|---------------------|----------|
| 4.1 | ✅ **Agent-to-agent delegation (crews)** — **SHIPPED 2026-08-05 (`b77e4ff` + follow-up `86c61e0`, §9):** DURABLE delegation — `AgentTask.parentTaskId` + DelegationService (spawnChild/childrenOf/tree/waitForChildren), REST delegate endpoints (owner/admin/team RBAC → 403), MCP `constellation.delegate_task`, portal Delegation tree + Delegate form; **follow-up: budget flow-down (tree aggregates descendants' tokens/cost) + result merging (`POST /engine/tasks/:id/merge` folds children's outcomes into the parent's result)**. LIVE: parent + 2 REST children + 1 MCP child all completed on Ollama; browser tree vision-verified; merge re-verified on the parent's result. Remaining: a portal-wide delegation view beyond the per-task dialog. | Multi-agent collaboration is the frontier — CrewAI (52K stars) does this but without a durable engine. LangGraph DeepAgents does planner+subagent. Constellation would do it DURABLY with a real engine underneath. | CrewAI (52K GitHub stars), LangGraph DeepAgents, Microsoft AutoGen | P0 |
| 4.2 | ✅ **Persistent agent memory (RAG + Graphify)** — **SHIPPED 2026-08-05 (`70e81a1` + `97d2383`):** docs-mode brain build (markdown via local Ollama embeddings) is the default; built graph bind-mounted to the api; **semantic retrieval layer live** — `/api/brain/search` embeds vault notes + graph labels with nomic-embed-text and ranks by cosine (zero new infra; pgvector/Chroma explicitly deferred as infra-heavy); agents remember via graph.ingest/remember → rebuild → node-appears. | "What did I do last time?" — without persistent memory, every agent is amnesiac. This is the single biggest differentiator for a 24/7 agent platform. | CrewAI's memory system, LangChain's memory, MemGPT, Letta | P0 |
| 4.3 | ✅ **MCP (Model Context Protocol) server + client** — **SHIPPED 2026-08-05 (`609da8a` server + `7e56229` client):** `POST /api/mcp` (JWT-guarded) JSON-RPC with 5 tools (list_tasks/run_task/engine_health/list_schedules/delegate_task); **the agent plane CALLS external MCP servers** via `MCP_CLIENT_URLS` (tools discovered live from tools/list, proxied through the worker). LIVE both directions: a task completed via MCP on Ollama; an agent called test.echo on an external server. | MCP is becoming the USB-C of AI tools. Anthropic's standard, now adopted by OpenAI, Google, and the open-source ecosystem. Being an MCP server AND client makes Constellation the universal agent hub. | Anthropic MCP, OpenAI's adoption, the growing MCP ecosystem | P0 |
| 4.4 | ✅ **Agent skill marketplace** — **SHIPPED 2026-08-05 (`0803ddd`, §9):** 7 packaged skills (cron + prompt + maxSteps); install = a real `skill:<id>` cron schedule; REST catalog/install/uninstall/toggle (admin-gated); portal `/skills` card grid. | The "app store" for AI agents. The killer feature: non-developers pick a skill, it runs. | Zapier's "Zaps," n8n's workflows, GitHub Actions marketplace | P1 |
| 4.5 | ✅ **Autonomous incident response** — **SHIPPED 2026-08-05 (`ee5b304` primitive + `ebef6dc` ingestion):** event-triggered workflows on engine.task.failed auto-remediate (proven live); **Prometheus/Grafana Alertmanager webhook ingestion** (`/api/alerts/webhook`, shared-secret guarded) turns firing alerts into `engine.alert.fired` events that spawn the remediation workflow — proven live end-to-end. | This is the "PagerDuty replacement" vision. AI agents as first-responders, humans as reviewers. Every SRE team wants this. | PagerDuty + AI, Datadog AI agent, but none self-hosted | P2 |
...
| 4.7 | ✅ **Audit + compliance reports** — **SHIPPED 2026-08-05 (`518a5b3` CSV + `095234e` PDF):** `/api/audit/export` — CSV (filters, RFC-4180) and **zero-dep PDF** (`?format=pdf`, multi-page audit table); portal Export CSV button on the Audit tab. | Every enterprise buyer will ask for this. The immutable audit trail is already built — make it reportable. | ServiceNow GRC, Vanta, Drata | P2 |
...
3. ~~**Prometheus metrics + health dashboard**~~ **COMPLETE** — `/api/metrics` DONE (`ac2cf11`), portal `/health` page DONE (`0647666`, LIVE-PROVEN in a browser), **pre-built Grafana dashboard JSON DONE (`0127ce1` — provisioned "Constellation Platform", 19 panels, LIVE-PROVEN with real data + the engine-metrics wiring fix)**.
4. ~~**Real SSO round-trip (Keycloak + Caddy)**~~ **DONE (`4d55928` — real Keycloak RS256 token → api 200, verifiers coexist, tampered → 401, portal /tools tiles + Caddy-proxied paths LIVE-PROVEN in a real browser).**
5. ~~**Plugin sandboxing**~~ **DONE (`5f268f3` — process-mode sandbox, OPT-IN via PLUGIN_SANDBOX_MODE/PLUGIN_SANDBOX_PLUGINS; timeout/heap/result caps + crash containment; LIVE-PROVEN: boom/crash/hang contained, graphify ran in the child).**
6. ~~**Worker as separate process**~~ **DONE (`3b37129` — ENGINE_WORKER_MODE=embedded|separate + dist/worker-main.js; LIVE-PROVEN: api enqueues without consuming, the worker completed the task + fired a cron schedule cross-process).**
7. ~~**Portal full `/engine` task UI**~~ **DONE (`b6e379f` — status filter tabs with counts, 2s live step streaming + pulsing 'live' badge, Re-run for finished tasks, model picker datalist from the health providers, Result panel with Copy; LIVE-PROVEN in a real browser: submitted a tool-calling task, dialog showed STEP HISTORY (3) → TOOL CALL graphify.graph.query → TOOL RESULT → DONE + RESULT JSON + Copy + filter tabs + Re-run).**
8. ~~**Plugin marketplace**~~ **DONE (`6e596af` — browse + install + uninstall with HOT-RELOAD, no restart; bundled plugins-catalog/ shelf + loader.reload() + marker-gated uninstall; LIVE-PROVEN: catalog → install → enabled + catalogInstalled + folder present → uninstall → back to available, all in a real browser with toasts).**
9. ~~**Visual workflow builder**~~ **DONE (`9e05c7a` — Workflow/WorkflowRun models + migration, definition validator + {{steps.<id>.result|error}} templating, run executor (agent steps via the engine queue, tool steps via plugin invoke, per-step crash-safe trail), CRUD + run + history routes guarded by core:workflow:manage, zero-dep drag-reorder builder UI on /workflows with live run trail; LIVE-PROVEN: 2-step agent→tool run completed with templated args + browser-created workflow ran to completion with the trail rendered).**
...
| **Multi-model compare (3.6)** | Polaris (solo) | Usage/cost PERSISTED on tasks (TokenBudget input/output/cost accumulation, worker persistUsage at every terminal path, migration add_task_usage) + portal /compare (same prompt on 2+ models, side-by-side latency/tokens/$ cards + summary table); nav entry | `f2afda4` | `e54cae6` | 513 → **516** (+3) | ✅ LIVE: ollama 51.0s/$0 vs deepseek-v4-flash 2.3s/$0.000018, persisted tokens both; browser results table + cards verified |
| **Notification channels (3.5 remainder)** | Polaris (solo) | Webhook channels (generic/Slack/Discord/Teams envelopes, per-kind filters, enabled toggle, fire-and-forget fetch+timeout), REST /notifications/channels + Test, NEW engine.task.completed/paused events, portal admin Channels tab | `88944b7` | `e54cae6` | 516 → **532** (+16) | ✅ LIVE: local listener — generic got completed+failed, slack only the failure, Test + browser Test delivered POSTs |
| **Workflow trigger wiring** | Polaris (solo) | ScheduledTask.workflowId (migration add_workflow_triggers) → scheduler RUNS workflows (fire-and-forget); WorkflowTriggerService cron auto-schedules + event listeners on core+platform scopes; bidirectional forwardRef; = autonomous incident-response primitive; 1 real test-found bug (deactivate-all-keys on trigger change) | `ee5b304` (+ fixup `4b426f8`) | `e54cae6` | 532 → **541** (+9) | ✅ LIVE: cron workflow auto-armed → fired → completed; PUT re-sync; doomed task fired engine.task.failed event workflow → completed |
| **Team spaces API (3.7, slice 1)** | Polaris (solo) | Organization/Team/TeamMember (owner\|admin\|member, migration add_team_spaces) + /api/teams RBAC (403 on non-manager), /me teams, AgentTask.teamId scoping (submit-into-team membership check; non-admin union list) | `418c0d8` | `e54cae6` | 541 → **554** (+13) | ✅ 38 files green; browser proof pending (next round) |
| **Team spaces portal (3.7, slice 2)** | Polaris (solo) | /teams page + TeamsView (create team → owner, my-teams role badges, members by email + role select, remove w/ owner protection), lib/teams.ts, core-teams nav; TaskService list projection + usage/cost; 1 live-found path bug (double /api prefix) + unhydrated-dev-server gotcha | `7216f45` | `ca78dbc` | 554 → **554** (web gates only) | ✅ LIVE: viewer /me teams + team task via ?teamId= + 403 member mgmt; browser OWNER badge + member rows + ADD MEMBER box verified |
| **SMTP email channel (3.5 remainder)** | Polaris (solo) | Zero-dep net/tls SMTP client (smtp-send.ts), channel type smtp (recipient/from, env relay), injectable sender, honest no-SMTP_HOST degrade, .env.example block; 2 real bugs found live (multi-line EHLO folding, 3xx DATA go-ahead) | `359ece8` | `ca78dbc` | 554 → **559** (+5) | ✅ LIVE vs local SMTP stub: test mail + real engine.task.failed mail (subject/body recorded literally) |
| **MCP server (4.3)** | Polaris (solo) | POST /api/mcp JSON-RPC 2.0 (initialize/tools/list/tools/call/ping/resources), 4 constellation tools (list_tasks/run_task/engine_health/list_schedules), JWT-guarded, error codes | `609da8a` | `ca78dbc` | 559 → **568** (+9) | ✅ LIVE client exchange: handshake + a REAL task completed via MCP on Ollama (mcp-ok, 424 tokens, $0) + isError/401 |
| **Compliance export (4.7)** | Polaris (solo) | GET /api/audit/export (CSV, actor/action filters, cap 1000), pure auditToCsv (metadata JSON, quoting tested), portal Export CSV button on the Audit tab | `518a5b3` | `ca78dbc` | 568 → **571** (+3) | ✅ LIVE: 200/2415-byte real CSV + workflow-filtered + 401; browser button toast |
| **Brain UX + docs-mode RAG (4.2 foundation)** | Polaris (solo) | Brain page: adaptive force-layout iterations, content-keyed layout memo (poll-jank fix), label anti-collision + zoom gating, degraded banner. RAG: GRAPHIFY_MODE=docs default, graphify-out bind mount (named-volume root cause), 7b model, Dockerfile [mcp,ollama]; 2 real failures fixed live (openai package missing, 1.5b hollow JSON) | `70e81a1` | `1d37eb5` | api unchanged; web gates clean | ✅ /brain NOT-BUILT UI browser-verified; extraction live (258 code + 11 docs); **graph materialization VERIFIED in close pass: 2.49MB / 2274 nodes / 4269 edges, stats available:true, query grounded, remember appends vault notes** |
| **Crews / task delegation (4.1)** | Polaris (solo) | Migration add_task_delegation (AgentTask.parentTaskId), DelegationService (spawnChild/childrenOf/tree/waitForChildren), REST /delegate + /children + /tree (owner/admin/team RBAC), MCP constellation.delegate_task, portal Delegation section in task detail | `b77e4ff` | `1d37eb5` | 571 → **579** (+8) | ✅ LIVE: parent + 2 REST children + 1 MCP child all completed on Ollama; viewer delegate 403; browser tree (4 tasks) + form vision-verified |
| **Skill marketplace (4.4)** | Polaris (solo) | 7-skill catalog, install = skill:<id> cron schedule, REST /api/skills (admin-gated; PermissionsGuard per-route found live), portal /skills card grid | `0803ddd` | `docs-commit-below` | 579 → **586** (+7) | ✅ LIVE: 2 installs → real schedules (correct crons) → toggle → uninstall; viewer 403; browser catalog vision-verified |
| **MCP client side (4.3 tail)** | Polaris (solo) | McpClientService (MCP_CLIENT_URLS/HEADERS, tools/list discovery cached, tools/call proxying), worker plugin="mcp" branch (prompt/invoke/approval) | `7e56229` | `docs-commit-below` | 586 → **593** (+7) | ✅ LIVE: agent called test.echo on an external MCP server (:9101) → echoed result, task completed |
| **Semantic retrieval (4.2 tail)** | Polaris (solo) | BrainService.search: vault notes + graph labels embedded via Ollama nomic-embed-text, cosine rank, 5-min cache, zero new infra; POST /api/brain/search | `97d2383` | `docs-commit-below` | 593 → **596** (+3) | ✅ LIVE: model-routing → EngineModule top hit; delegation → remembered vault note TOP hit (RAG loop closed) |
| **PDF compliance reports (4.7 tail)** | Polaris (solo) | Zero-dep PDF writer (valid object graph, multi-page, WinAnsi escape); /api/audit/export?format=pdf | `095234e` | `docs-commit-below` | 596 → **599** (+3) | ✅ LIVE: 200 / 10,788 bytes, %PDF-1.4, 2 pages for 50 rows, real auth.login rows |
| **Alert-trigger ingestion (4.5 tail)** | Polaris (solo) | AlertWebhookService: Alertmanager payload → engine.alert.fired events + audit; POST /api/alerts/webhook (shared-secret) | `ebef6dc` | `docs-commit-below` | 599 → **602** (+3) | ✅ LIVE: alert-remediation workflow armed + ran on a real webhook (run 1 completed); wrong secret 401 |
| **Crews follow-up (budget + merge)** | Polaris (solo) | Tree aggregates descendants' tokens/cost; mergeResults folds children's outcomes into the parent's result; POST /engine/tasks/:id/merge; portal budget badge + Merge button | `86c61e0` | `docs-commit-below` | 602 → **604** (+2) | ✅ LIVE: tree aggregation (1 child, 1347 tok, $0) + merge wrote children payload into parent's result |

**Final tree at `ca78dbc`:** clean (only untracked `Prompt to Clau_Partner.txt`).
...
- **2026-08-05 — 👥 PHASE 4.0 — CREWS FOLLOW-UP (budget flow-down + result merging): DONE, gate-verified, LIVE-PROVEN (git `86c61e0`, local only). Polaris (solo).** `DelegationService.tree()` now aggregates descendants' usage onto the root (`childCount`/`childrenTotalTokens`/`childrenCostUSD` — the orchestrator sees what its crew spent; children already inherit the parent's maxTokens at spawn); `mergeResults(parentId)` collects every descendant's {title, status, result, tokens, cost} and writes `{summary: "Merged N sub-agent result(s)…", children: [...]}` onto the parent task's `result`; REST `POST /engine/tasks/:id/merge` (owner/admin/team visibility); portal Delegation section gains the budget badge + owner-gated **Merge results** button. LIVE: fresh parent completed → child completed (1347 tok) → tree root childCount 1 / 1347 tok / $0 → merge wrote the payload into the parent's result (re-fetch verified). api 602→604.
- **2026-08-05 — 🚨 PHASE 4.0 item 4.5 tail — PROMETHEUS/GRAFANA ALERT-TRIGGER INGESTION: DONE, gate-verified, LIVE-PROVEN (git `ebef6dc`, local only). Polaris (solo).** `AlertWebhookService` turns Alertmanager webhook payloads into normalized alerts + **`engine.alert.fired`** bus events (core scope) + audit rows — the missing trigger source for the autonomous incident-response loop (event-triggered workflows arm on any core event). `POST /api/alerts/webhook` (public; `X-Webhook-Secret` vs `ALERT_WEBHOOK_SECRET`; unset = dev-accept documented); empty payloads → []; bus failures never break ingestion. LIVE: "alert-remediation" workflow (event trigger engine.alert.fired) auto-armed; a real Alertmanager payload (HighCPU critical + DiskFull warning) ingested → **the workflow RAN** — run 1 completed, run 2 failed on the 7b model's maxSteps rambling (mechanics identical, honest note); wrong secret → 401. api 599→602.
- **2026-08-05 — 📄 PHASE 4.0 item 4.7 tail — PDF COMPLIANCE REPORTS: DONE, gate-verified, LIVE-PROVEN (git `095234e`, local only). Polaris (solo).** Zero-dependency PDF writer (`pdf-export.ts`): minimal but fully valid PDF (catalog/pages/page/fonts/content streams, xref+trailer), audit table (WHEN/ACTOR/ACTION/METADATA), multi-page row flow, WinAnsi-safe sanitize + `\(`/`\)`/`\\` escaping; `GET /api/audit/export?format=pdf` (CSV stays default). LIVE: 200 / 10,788 bytes, `%PDF-1.4` header, 2 pages for 50 rows, real auth.login rows drawn. api 596→599.
- **2026-08-05 — 🔎 PHASE 4.0 item 4.2 tail — SEMANTIC RETRIEVAL LAYER: DONE, gate-verified, LIVE-PROVEN (git `97d2383`, local only). Polaris (solo).** The pgvector/Chroma alternative with ZERO new infra: `BrainService.search` embeds the query + an index (vault `##` sections + all graph node labels) via local Ollama `nomic-embed-text` (`/api/embed`, batched 64), ranks by cosine (pure exported `cosineSimilarity`, 0.16 threshold, top-8), 5-min index cache, honest `unavailable:true` degrade; `POST /api/brain/search` (BRAIN_READ); `BRAIN_EMBED_MODEL` env. LIVE: model-routing query → EngineModule top hit; delegation query → **the remembered "Crews 4.1 record" vault note is the TOP hit** — remember → index → semantic retrieval loop closed. First index build ~48s (2274 labels on CPU), cached after. api 593→596.
- **2026-08-05 — 🔌 PHASE 4.0 item 4.3 tail — MCP CLIENT side: DONE, gate-verified, LIVE-PROVEN (git `7e56229`, local only). Polaris (solo).** `McpClientService` (core/engine): `MCP_CLIENT_URLS` (alias=url, comma-separated) + optional `MCP_CLIENT_HEADERS`; each server's `tools/list` discovered at worker-prompt build (30s cache) and surfaced as `plugin="mcp" tool="<alias>.<name>"`; the worker proxies agent args via JSON-RPC `tools/call` and returns content text; unreachable servers degrade to no-tools. Worker integration: prompt lines, executeToolCall branch → invokeMcpTool (same ToolResult shape), approval gate skips per-tool flags for remote tools (approve→resume re-enters the worker). LIVE: agent task called `test.echo` on a local external MCP server (:9101) → tool_result `echoed: hello-from-constellation`, task completed (3 steps, 1108 tok); first probe's fetch-failed documented as the server-down degrade path. api 586→593.
- **2026-08-05 — 🧰 PHASE 4.0 item 4.4 — SKILL MARKETPLACE: DONE, gate-verified, LIVE-PROVEN (git `0803ddd`, local only). Polaris (solo).** `SkillService` + 7-skill catalog (daily PR triage, weekly dependency audit, SSL expiry monitor, nightly health check, weekly cost report, daily incident summary, weekly code review digest) — each a packaged (cron + prompt template + maxSteps); **install = a real `skill:<id>` cron scheduled task** (no new tables; the scheduler runs it). REST `GET /api/skills` (catalog + install state + nextRunAt) · install/uninstall/toggle admin-gated (PermissionsGuard + core:plugin:manage — **found live: the guard must be attached per-route**); portal `/skills` card grid (category chips, cron, state badges, next-run, Install/Resume/Uninstall) + `core-skills` nav. LIVE: 2 installs → real schedules with correct crons (0 8 * * * / 0 7 * * *), toggle → paused, uninstall → removed, viewer install 403, browser catalog vision-verified. api 579→586.
- **2026-08-05 — 👥 PHASE 4.0 item 4.1 — CREWS / TASK DELEGATION: DONE, gate-verified, LIVE-PROVEN (git `b77e4ff`, local only). Polaris (solo).** Durable multi-agent delegation: migration `add_task_delegation` (`AgentTask.parentTaskId` self-relation, SET NULL, indexed — the tree is a column, not an in-memory map); `DelegationService` — `spawnChild` (inherits actor/team/token budget, maxSteps default parent−2, link + enqueue), `childrenOf`, `tree` (depth 4 / 50 nodes), `waitForChildren` (bounded poll), no-DB degrade; **any existing task can spawn children** (a failed orchestrator can grow a crew — recovery pattern); REST `GET /engine/tasks/:id/children` · `GET /tree` · `POST /delegate` (owner/admin/team visibility → 403); **MCP tool `constellation.delegate_task`** (spawn + bounded wait — MCP clients orchestrate crews); portal task-detail dialog **Delegation section** (live indented tree with status badges + token counts, Delegate form owner-gated; StatusBadge extracted to a shared file). LIVE: parent `crews-parent` completed → 2 REST children completed (878/873 tok, $0) → viewer delegate **403** → MCP child created AND completed (1371 tok) → browser tree shows all four tasks (vision-verified, "4 tasks in this tree" + form). 3 real bugs fixed live: DI trap #3 twice (`import type` erased PrismaService + TaskService metadata → boot failure "Nest can't resolve"), a stray `],` in EngineModule providers, and the stale-dist gotcha (`boot-api-v0.3.sh` runs `node dist/main.js` — source edits need a rebuild). api 571→579 (41 files). **RAG tail verified in the same pass:** graph.json materialized (2,490,148 bytes; 2274 nodes / 4269 edges), `/api/brain/stats` available:true, `/api/brain/query` returns grounded graph answers, `/api/brain/remember` appends vault notes (`brain/notes/`).
- **2026-08-05 — 🧠 PHASE 4.0 — BRAIN-PAGE UX FIXES + DOCS-MODE (RAG) GRAPHIFY WIRING: DONE, gate-verified, LIVE-PROVEN (git `70e81a1`, local only). Polaris (solo).** Two coupled fixes. Brain page: `force-layout.ts` ADAPTIVE iterations (150/220/300 by node count — the O(n²) pass at the 300-node cap no longer blocks the main thread); `brain-graph-view.tsx` layout keyed on CONTENT not the polled graph-object identity (the 1200+-node jank — physics re-ran every poll tick), LABEL ANTI-COLLISION + zoom gating (hub labels only ≥0.9 zoom, yield within 46px of a placed label; selected/highlighted always win), DEGRADED banner when the 300-node cap truncates. RAG: the graphify sidecar's docs-mode (markdown via local Ollama embeddings) is now the DEFAULT — compose `GRAPHIFY_MODE=docs`, graphify-out BOUND to `./graphify-out` (the NAMED VOLUME was the "brain not built yet" root cause — the host api never saw the built graph), `GRAPHIFY_MODEL=qwen2.5-coder:7b` (1.5b emits hollow/unparseable JSON — every chunk bisected into failure), `OLLAMA_API_KEY=local` placeholder (CLI demands it even for a local backend), Dockerfile `graphifyy[mcp,ollama]` (docs backend needs the `openai` client), nomic-embed-text pulled. **FINAL MATERIALIZATION VERIFIED 2026-08-05 (close pass):** graph.json = 2.49 MB (2274 nodes / 4269 edges); `/api/brain/stats` available:true; `/api/brain/query` real grounded answer (ModelRouterHealth etc.); `/api/brain/remember` ok → vault note appended. Evidence `artifacts/brain-rag/`.
- **2026-08-05 — 📄 PHASE 4.0 — COMPLIANCE EXPORT: DONE, gate-verified, LIVE-PROVEN (git `518a5b3`, local only). Polaris (solo).** `GET /api/audit/export?actor=&action=&limit=` — RFC-4180 CSV download (Content-Disposition attachment, admin-only); `AuditService.listForExport` (substring filters, cap 1000) + pure `auditToCsv` (metadata JSON-encoded; quoting unit-tested). Portal Export CSV button on the /notifications Audit tab (blob download + toast). LIVE: 200/2415-byte real CSV (auth.login rows with metadata), workflow-filtered export, 401 unauthenticated; browser button → toast. api 568→571.
- **2026-08-05 — 🔌 PHASE 4.0 — MCP SERVER: DONE, gate-verified, LIVE-PROVEN (git `609da8a`, local only). Polaris (solo).** `POST /api/mcp` (JWT-guarded) speaking the MCP 2025-03-26 request/response subset (initialize / notifications/initialized / ping / tools/list / tools/call / resources/list; JSON-RPC errors −32600/1/2/3). Tools: `constellation.list_tasks` (status/teamId filters), `run_task` (submit + enqueue + bounded poll → terminal record with persisted usage/cost), `engine_health` (full payload), `list_schedules`. EngineModule exports EngineAvailabilityService + ModelRouterService for it. LIVE (real client exchange): handshake, 4 tools, run_task COMPLETED a real Ollama task via MCP (result mcp-ok, stepCount 1, 424 tokens, $0), unknown tool isError, no-auth 401. api 559→568.
- **2026-08-05 — 📧 PHASE 3.0 item 3.5 remainder — SMTP EMAIL CHANNEL: DONE, gate-verified, LIVE-PROVEN (git `359ece8`, local only). Polaris (solo).** Zero-dep SMTP client (`smtp-send.ts` — node net/tls: EHLO/AUTH PLAIN/MAIL/RCPT/DATA/QUIT, multi-line folding, 3xx DATA go-ahead, timeout); channel type `smtp` (recipient + optional from), env-configured relay (SMTP_HOST/PORT/USER/PASS/FROM/SECURE in .env.example), injectable sender for tests, honest degrade when SMTP_HOST unset, fire-and-forget like webhooks; controller validates the recipient email. LIVE vs a local SMTP stub: test message + REAL `engine.task.failed` mail recorded literally (subject `[Constellation] ERROR — Task failed`, body with the Ollama 404 detail, kind/severity/task reference). 2 real bugs found live (multi-line EHLO folding, 3xx regex). api 554→559.
- **2026-08-05 — 🏢 PHASE 3.0 item 3.7 — TEAM SPACES, PORTAL SLICE: DONE, gate-verified, LIVE-PROVEN (git `7216f45`, local only). Polaris (solo).** Completes the round whose API shipped at `418c0d8`. Portal `/teams` page + TeamsView (create team → owner; my-teams list with role badges; team detail with member emails/roles; add-member-by-email with role select; remove member — trash hidden for the owner; management controls only for owner/admin — the API enforces, the UI hides), `lib/teams.ts` client, `core-teams` nav (Users, order 25). Fix: `TaskService.findAll` projection now includes the persisted usage/cost fields. LIVE: admin creates core-team → adds viewer → viewer `/me` carries the team, sees the team task via `?teamId=`, POST members → 403; browser teams list (OWNER badge) + detail (both members, ADD MEMBER box) vision-verified. **TEAM SPACES (3.7) COMPLETE end-to-end.**
- **2026-08-05 — 🏢 PHASE 3.0 item 3.7 — TEAM SPACES, API SLICE: SHIPPED (git `418c0d8`, local only). Polaris (solo).** Org→Team→User multi-tenancy foundation: `Organization`/`Team`/`TeamMember` models (role owner|admin|member, migration `add_team_spaces`), `TeamService` (create/list/detail/addMember/removeMember/canManage/isMember, owner-protected removal, no-DB empty degrade), REST `/api/teams` (JWT; member management owner/admin-only → 403), `/api/auth/me` now carries the caller's teams, and REAL resource scoping: `AgentTask.teamId` — submitting into a team requires membership, `GET /engine/tasks?teamId=` filters, non-admins see personal + their teams' tasks (OR-union query). api tests 541→554. **Remaining in-round: portal /teams page + live browser proof** (next session's first task). Follow-ups: team-scoped schedules/workflows/plugins, org roles, invitations.
- **2026-08-05 — 🔀 PHASE 3.0 — WORKFLOW TRIGGER WIRING + AUTONOMOUS INCIDENT RESPONSE: DONE, gate-verified, LIVE-PROVEN (git `ee5b304` + fixup `4b426f8`, local only). Polaris (solo).** The workflow builder stored `trigger` definitions but nothing consumed them. Now: `ScheduledTask.workflowId` (migration `add_workflow_triggers`) — firing a workflow schedule RUNS the workflow (WorkflowRunService, fire-and-forget so the poll loop never blocks) instead of enqueuing a task; `WorkflowTriggerService` reconciles on create/update/delete: cron → auto-managed `workflow:<id>` schedule (remove+recreate, PUT re-sync proven), event → listeners on BOTH core (`engine.task.*` — the incident-response pattern) and platform (plugin lifecycle) scopes with active-set deactivation; scheduler↔workflows module cycle via bidirectional `forwardRef`. LIVE: cron workflow auto-armed → fired at the minute boundary → completed on Ollama; a real doomed task fired the `engine.task.failed` event workflow → completed. api 532→541.
- **2026-08-05 — 📨 PHASE 3.0 item 3.5 remainder — NOTIFICATION CHANNELS: DONE, gate-verified, LIVE-PROVEN (git `88944b7`, local only). Polaris (solo).** Webhook channels (generic / Slack / Discord / Teams envelope dialects), per-channel `kinds` filters + enabled toggle, stored under one core settings key; fire-and-forget delivery via global fetch + AbortSignal.timeout — a failing webhook never breaks the feed; REST `/api/notifications/channels` (list/upsert/delete/test, 400/404 honest); NEW `engine.task.completed` + `engine.task.paused` events (worker + EngineAlertService) so the 'completed/failed/needs-approval' vision is real; portal admin **Channels** tab (add form with kind checkboxes, Test with toast, Remove). LIVE: local listener on :9080 — generic channel got completed+failed, slack-shaped channel got ONLY the failure (kind filter), test endpoint + browser Test button delivered real POSTs. api 516→532.
- **2026-08-05 — ⚖️ PHASE 3.0 item 3.6 — MULTI-MODEL COMPARE / A/B: DONE, gate-verified, LIVE-PROVEN (git `f2afda4`, local only). Polaris (solo).** Closes the v0.3 follow-up (per-call usage was in-memory only): `TokenBudget` now accumulates input/output tokens + derived cost; the worker persists cumulative usage at EVERY terminal path (`AgentTask.inputTokens/outputTokens/totalTokens/estimatedCost`, migration `add_task_usage`). Portal `/compare`: pick 2+ models (from health providers), run the same prompt, live side-by-side cards + summary table (model | status | latency from startedAt→completedAt | tokens | cost), result copy. LIVE: same prompt on ollama `qwen2.5-coder:7b` (51.0s, $0) vs deepseek-v4-flash (2.3s, $0.000018) — persisted token/cost fields on both task records + browser results table verified. api 491→516.
- **2026-08-04 — 🔔 PHASE 3.0 item 3.4 — NOTIFICATION CENTER (durable platform event feed + portal UI): DONE, gate-verified, LIVE-PROVEN (git `35a76fe`, local only). Polaris (solo round).** The platform had events (engine alerts ring buffer, scheduler, audit) but no durable, queryable feed and no UI — "what happened while I slept" was unanswerable. This round builds the notification center: events → durable `notifications` table → REST → portal page + sidebar unread badge.
  - **API (`apps/api/src/core/notifications/`):** `Notification` Prisma model (id, kind, severity, title, message, refType/refId, read, createdAt; migration `20260804184322_add_notifications`, applied + committed). `NotificationService` — subscribes on `onModuleInit` to the EventBus "core" topics `engine.task.failed|stale|recovered` (v0.5 alerts) and the NEW scheduler topics `scheduler.schedule.fired|error`, and persists each event (no-DB → warn-once empty feed, same degrade discipline as every core service; text clamped to 500 chars). `NotificationsController` — `GET /api/notifications` (?limit≤100, ?kind, ?unread → `{items, unreadCount}`), `GET /unread-count`, `POST /read-all`, `POST /:id/read`, `DELETE /:id` (404s for missing); global JwtAuthGuard (any authenticated user — per-user targeting is a team-spaces follow-up). **Scheduler now EMITS** (`scheduler-engine.service.ts`): `scheduler.schedule.fired` after a successful fire, `scheduler.schedule.error` on sweep/event-fire failure — guarded `emitSchedulerEvent()` (no bus = silent, never throws). `NotificationsModule` wired into `app.module.ts`. **22 new tests** (19 notification service: record/clamp/no-DB/list+filters/unread/markRead/markAll/dismiss/bus-registration + the 5 topic→row mappings incl. unknown-topic ignore; 3 scheduler emission tests). api suite **491 → 513**.
  - **🐛 REAL BUG #1 found at boot (the documented NestJS DI trap #3):** `import type { PrismaService }` erased the runtime metadata → `Nest can't resolve dependencies of the NotificationService (?, EventBusService) … argument Function at index [0]` — the live boot failed while every offline `new NotificationService(...)` test stayed green. Fixed: VALUE import (same class as the OTel round; comment left at the import).
  - **🐛 REAL BUG #2 found in the browser (invisible to gates):** the audit tab's lazy-load effect listed `auditLoading` in its dependency array — `setAuditLoading(true)` re-ran the effect, the cleanup flipped `active=false`, and the in-flight fetch result was discarded → the tab spun on "Loading audit trail…" forever. Fixed by removing the state from the deps (+ explanatory comment). Only the browser proof catches this class.
  - **Portal:** `/notifications` page + `NotificationsView` — Feed tab (10s poll; filter chips All/Unread/Tasks/Schedules; severity icons error/warning/success/info; click-to-mark-read optimistic; dismiss ✕; "Mark all read" (guarded when unread=0); honest empty state "You're all caught up."; task refs link to /engine) + Audit log tab (admin-only, lazy-loads `GET /api/audit`, forbidden state for non-admins). `lib/notifications.ts` never-throw client incl. `fetchAuditEntries`. Sidebar: `core-notifications` nav entry (Bell, order 55) + `UnreadBadge` polling `unread-count` every 20s + on window focus. DESIGN_SKILL language (surface rows, Reveal, press-scale, both themes). Web tsc clean, lint 0 errors (17 pre-existing warnings).
  - **LIVE PROOF (evidence in `artifacts/notifications/`):** all **5 event topics** proven live end-to-end in ONE embedded boot (the EventBus is in-process — embedded is the honest mode; separate-worker would split-brain the feed): `scheduler.schedule.fired` (real cron auto-fire), `engine.task.failed` (unknown-model → Ollama 404 → terminal), `engine.task.stale` + `engine.task.recovered` (supervisor recovered a crafted stale row while both worker slots were busy — message rendered literally `No progress for 3602110ms`). Repetition-proof: the `* * * * *` cron fired 20× while the api stayed up and every fire was persisted. REST: unread 4 → mark-read → read-all (3) → unread 0 → dismiss → 404 → no-token 401 → kind+unread filtered query. **Real browser (CDP):** sidebar badge = 4, feed rows + severity dots + refs, filter chips, Mark all read cleared the badge (`wait` on badge-gone → true), Audit log tab rendered real entries (auth.login, engine.schedule.created/deleted, workflow.run…). 3 screenshots, vision-verified.
  - **Gates (final pass on the left-behind tree):** lint/build/typecheck/test **20/20**, 0 cached · tests **615 → 637** (api **491 → 513**: +22). Tree clean after docs. Nothing pushed. $0/local unbroken. **Next: multi-model compare → team spaces.**

- **2026-08-04 — 🧩 PHASE 3.0 — VISUAL WORKFLOW BUILDER (compose + run multi-step agent/tool pipelines): DONE, gate-verified, LIVE-PROVEN (git `9e05c7a`, local only). Polaris (solo round).** Workflows compose the EXISTING engine primitives into repeatable pipelines:
  - **API:** `Workflow` + `WorkflowRun` Prisma models (migration `20260804174024_add_workflows`, applied + committed). `workflow.schema.ts` — the WorkflowDefinition validator (trigger manual|cron|event with conditional fields, steps agent|tool, duplicate-id/required-field checks, ≤20 steps) + `{{steps.<id>.result|error}}` templating renderer. `workflow.service.ts` CRUD (400 on invalid definitions). `workflow-run.service.ts` — the executor: agent steps create + enqueue an engine task and poll to terminal (bounded 10min, injectable `waitForTask` seam for tests); tool steps invoke PluginToolService as a trusted system caller (`platform:admin`); the per-step outcome trail is persisted after EVERY step (crash-safe record); the run stops at the first failing step with an honest error; results pipe into later steps via templating (the SDK ToolResult's `data` payload key is captured — graphify returns `{raw,text,tool}`). `WorkflowsController` CRUD + `POST /:id/run` + run history — guarded by the new `CorePermissions.WORKFLOW_MANAGE` (`core:workflow:manage`; `platform:admin` implies it, so no seed change), mutations audited. **18 new tests** (validator 12 + executor 6 with fake seams mirroring the REAL wire envelopes). api suite **473 → 491**.
  - **🐛 One real bug found live:** the executor checked `invocation.outcome === "ok"`, but the ToolInvocation completed envelope's outcome is **"completed"** — every tool step was marked failed with an empty error. Fixed; the test stubs had mirrored the WRONG envelope (`outcome:"ok"`) and missed it — corrected to the real shapes (and the rejection envelope's reason/message now surfaces as the step error).
  - **Portal:** `/workflows` page + `WorkflowsView` — the zero-dep visual builder: trigger picker (Manual/Cron/Event), step canvas with native drag-to-reorder + move buttons, inline editing (agent prompt/model/maxSteps; tool plugin/tool/args JSON), add Agent/Tool steps, Save/Create, Run with a live-polling run trail (per-step ok/failed badges + step summaries). `lib/workflows.ts` never-throw client; Workflows nav entry (Workflow icon). Web tsc clean, eslint 0 problems.
  - **LIVE PROOF (evidence in `artifacts/workflows/`):** created a 2-step workflow (agent "Produce" → graphify tool "Consume" with `{"question": "{{steps.s1.result}}"}`) → run → **completed**: step 1 agent ok (4.2s, real engine task through the queue, result `{"summary":"workflow-first"}`), step 2 tool ok (37ms, graphify consumed the TEMPLATED result); invalid definition → **400**; no token → **401**; tool-step payload capture verified live (result `{raw,text,tool}`). Browser (CDP): the builder rendered (TRIGGER Manual/Cron/Event, Agent step card, add-step palette, drag handles), a workflow created in the UI ran to completion (~90s on CPU Ollama) and its trail rendered in RECENT RUNS with green completed + ok badges (screenshot). Cron/event triggers are stored in the definition; wiring them to the scheduler is the documented follow-up.
  - **Gates:** api build + typecheck green, api suite **491/491**, web tsc + lint clean. Repo total **615** (api 491, sdk 21, graphify 40, browser-use 47, cli 16). Tree clean after docs. Nothing pushed. $0/local unbroken.

- **2026-08-04 — 🛒 PHASE 3.0 — PLUGIN MARKETPLACE (browse + install + uninstall, hot-reload): DONE, gate-verified, LIVE-PROVEN (git `6e596af`, local only). Polaris (solo round).** The /modules page is now a real marketplace on top of the existing plugin registry — local-first (no remote registry needed, honest $0 path):
  - **API:** `PluginCatalogService` lists bundled-but-not-installed plugins from a new repo-relative `plugins-catalog/` folder (`PLUGINS_CATALOG_DIR`, default "plugins-catalog"); `install()` copies the bundle into `plugins/` + writes a `.constellation-installed.json` marker; `uninstall()` is **marker-gated** — a hand-placed plugin is NEVER removed (400), unknown id 404, already-installed 409. `PluginLoaderService.reload()` = clear + full re-scan (extracted `loadAll()`), so install/uninstall take effect **WITHOUT a restart** (registry gained `clear()`). `PluginsController`: `GET /plugins/catalog` (@Public) → `{ installed, available }`; `POST /plugins/:id/install` + `:id/uninstall` (PLUGIN_MANAGE, audited); list rows gain `catalogInstalled`. **11 new unit tests** (tmp-dir fs contract: entries/available/install/uninstall/markers/404/409/400/root resolution). api suite **462 → 473**.
  - **Portal:** `MarketplaceSection` on /modules — "Installed from the marketplace" zone (Uninstall buttons, only for catalog-installed) + "Available in the catalog" zone (Install buttons); mutations toast + re-fetch, the installed grid above picks the change up on its own 5s live poll. `lib/api.ts` gained `getPluginCatalog`/`installCatalogPlugin`/`uninstallCatalogPlugin` (never-throw); `PluginSummary` gained `catalogInstalled`. **Verification:** web tsc clean, eslint 0 problems.
  - **Bundled demo:** `plugins-catalog/hello-catalog` (manifest v2, definePlugin, SDK symlink, hand-written dist) — the marketplace's installable reference.
  - **🐛 One real bug found live:** the "Installed from the marketplace" zone cross-referenced the AVAILABLE list — but an installed plugin LEAVES `available`, so the zone rendered empty after a successful install. Fixed: derive the zone from the INSTALLED list (`catalogInstalled`). The browser run caught it.
  - **LIVE PROOF (evidence in `artifacts/marketplace/`):** catalog before = 3 installed (browser-use/graphify/hello-world, `catalogInstalled:false`) + `[hello-catalog 0.1.0]` available → POST install → hello-catalog **enabled** with `catalogInstalled:true`, folder present in plugins/, available empty (**hot reload — no restart**) → POST uninstall → folder gone, back to available. **Real browser (CDP):** /modules showed the marketplace shelf (Hello Catalog card + Install), clicking Install moved the card to INSTALLED FROM THE MARKETPLACE with an Uninstall button + live toast "Hello Catalog installed — the registry reloaded — it is now live"; clicking Uninstall moved it back to the available shelf. Screenshots captured.
  - **Gates:** api build + typecheck green, api suite **473/473** (32 files), web tsc + lint clean. Repo total **597** (api 473, sdk 21, graphify 40, browser-use 47, cli 16). Tree clean after docs. Nothing pushed. $0/local unbroken.

- **2026-08-04 — 🚀 PHASE 3.0 item 3.1 — PORTAL FULL `/engine` TASK UI (P0 — the #1 UX gap): DONE, gate-verified, LIVE-PROVEN (git `b6e379f`, local only). Polaris (solo round).** The engine page was functional but shallow (submit form, table, drawer, cancel/approve/reject, health strip). This round makes it a real task console — pure client-side depth over the existing REST contract (NO API changes):
  - **Status filter tabs** — All/Queued/Running/Paused/Completed/Failed/Cancelled with live counts (new `filter` state + `filteredTasks`; tab row above the table, aria-pressed).
  - **Live step streaming** — the open task's detail poll dropped from the 5s list cadence to **2s**, so the ReAct loop streams in as it runs; a pulsing accent **'live' badge** renders in the dialog while the task is non-terminal.
  - **Re-run** — terminal tasks (completed/failed/cancelled) get a Re-run button that fetches the detail and re-submits the SAME title/prompt/model as a fresh run (composition of the existing detail + submit calls — no new API surface).
  - **Model picker** — the model input is now a `<datalist>` fed by the health payload's provider list (ollama/deepseek/openrouter ids, with reachability labels); free text still works.
  - **Result panel** — completed tasks render the final result (pretty JSON) with a **Copy** button (`navigator.clipboard`, silent on denial).
  - **Verification:** web `tsc --noEmit` clean; eslint **0 problems** on the changed file (the lane's full gate — no web unit suite exists). **LIVE-PROVEN** in a real browser (CDP, `scripts/flow-engine-p3.json`, evidence in `artifacts/engine-p3/`): logged in → /engine → filter tabs rendered → submitted a tool-calling task → opened the dialog while it ran → captured the completed task showing **STEP HISTORY (3) — TOOL CALL graphify.graph.query → TOOL RESULT → DONE "engine-p3 proof OK" — plus the RESULT block with the final JSON `{"summary": "engine-p3 proof OK"}` and the Copy button, filter tabs (All/Queued/Running/Completed) + Re-run visible on the list**. Two live-found proof-script lessons: (1) the web app defaults to `:4000` for the API — boot web dev with `NEXT_PUBLIC_API_URL=http://localhost:4001/api` (no boot script existed; noted in §8); (2) step labels render UPPERCASE via CSS `text-transform`, so flow waits must match the RENDERED text, not the JSX casing.
  - **Gates:** web typecheck + lint clean (the only changed package this round); repo test suite untouched (586). Tree clean after docs. Nothing pushed. $0/local unbroken.

- **2026-08-04 — 📊 PHASE 2.0 item 2.3 — PRE-BUILT GRAFANA DASHBOARD + live engine-metrics wiring: DONE, gate-verified, LIVE-PROVEN (git `0127ce1`, local only). Polaris (solo round).** The last Phase 2.0 item: a provisioned dashboard over the real `/api/metrics` Prometheus metrics — the /health page + /api/metrics existed, but the Grafana dashboard file did not.
  - **What shipped:** `infra/grafana/provisioning/dashboards/constellation.json` — "Constellation Platform" (uid `constellation`, 19 panels: queue waiting/active/failed + uptime + alert-trail stat row; task lifecycle rate + engine alerts rate; model calls by provider, tokens by kind, spend USD, latency p95 (histogram_quantile); plugin tool calls by plugin/outcome; HTTP requests by status + latency p95; auth logins; schedule runs by outcome + scheduler due/listeners; supervisor events). `dashboards.yml` provider (auto-provision from the folder). Datasource `uid`s pinned (`prometheus`/`loki`/`tempo`) so dashboard refs are stable. `scripts/build-grafana-dashboard.py` is the reproducible generator. `infra/prometheus/prometheus.yml`: stale "no /metrics yet" comment corrected + a `constellation-api-host` job scraping `host.docker.internal:4001` so a HOST-booted dev api is observable by the in-docker stack. `scripts/cdp-browser.mjs` gained a `headers` flow act (`Network.setExtraHTTPHeaders`) — needed because Grafana 11's JSON login rejects valid creds (CSRF-era behavior) and modern Chrome strips userinfo from URLs; basic-auth headers are the browser path.
  - **🐛 TWO REAL BUGS found live by the data chain (offline gates could not catch):** (1) **unit-suffix duplication** — `constellation_process_uptime_seconds` declared with `unit: "seconds"` emitted `constellation_process_uptime_seconds_seconds` (same for both duration histograms → `_ms_milliseconds`); the declared names never matched the scraped samples, so EVERY uptime/latency panel was empty. Fix: drop the redundant `unit` on the three name-embeds-unit declarations (the registry's append behavior is test-pinned for real unit-less names). (2) **the engine counters were declared but never fed** — zero callers for `recordTaskLifecycle`/`recordModelCall`/`recordPluginToolCall`/`recordScheduleRun`/`recordSupervisor`/`recordAuthLogin` anywhere in the codebase (the metrics round only proved `/api/metrics` returns 200). Wired them via trailing `@Optional() MetricsService` (VALUE import — DI token) into TaskService (submitted/started/completed/failed/cancelled), ModelRouterService (calls/latency/tokens/cost, incl. the Ollama-fallback path), PluginToolService (plugin/tool/outcome), SchedulerEngineService (fired/error), SupervisorService (staleFound/recovered/failedStalled), AuthService (success/failure). **462 api tests stay green** (positional constructions untouched; the record calls are one-liners behind the optional seam — covered by this round's live proof, not new unit tests).
  - **LIVE PROOF (evidence in `artifacts/grafana-dashboard/`):** grafana recreated with provisioning → Grafana API serves the dashboard (19 panels, uid constellation; datasources prometheus/loki/tempo). Prometheus recreated with the host job → `constellation-api-host` target **up**, real samples after a login + a tool-calling task: task_lifecycle 3, model_calls 1, tokens 2, plugin_tool_calls 1, auth_logins 1, uptime 1, http duration histogram 72 buckets, model latency histogram 10 buckets. **Browser (CDP, basic-auth headers):** the dashboard renders live — task lifecycle / model calls / tokens spikes, uptime 56s, stat row 0s; the only "No data" panels are the HONEST empty states (no alerts fired; zero spend on local Ollama — costUSD only flows from cloud providers).
  - **Gates (`--force --concurrency=1`):** lint/build/typecheck/test **20/20**, 0 cached · tests **586** (api **462** — unchanged; infra + one-liner wirings). Tree clean after docs. Nothing pushed. $0/local unbroken. **PHASE 2.0 IS COMPLETE** (all 8 items + the 2.3 tail).

- **2026-08-04 — 🛡️ PHASE 2.0 item 2.7 — PLUGIN SANDBOXING (process mode): DONE, gate-verified, LIVE-PROVEN (git `5f268f3`, local only). Polaris (solo round).** Plugins ran in-process with full Node privileges (contractual isolation only). This round adds REAL OS-level isolation for tool invocations, operator-controlled and off by default.
  - **Design (secure defaults):** a child-Node-process sandbox — vm2 is deprecated (unmaintained, escape CVEs), isolated-vm needs native builds, so a spawned `node` child gives real crash/time/memory isolation with ZERO new deps (the repo's standing zero-dep stance). `scripts/plugin-sandbox-runner.mjs` imports the plugin entry and calls `invokeTool` with a MINIMAL context (**config + logger only — no db/events/memory: a sandboxed tool cannot reach the platform's data or event bus**; env fallbacks work because the child inherits the parent env). `PluginSandboxService` enforces: wall-clock timeout (`PLUGIN_SANDBOX_TIMEOUT_MS`, default 30s → SIGKILL), heap cap (`--max-old-space-size`, `PLUGIN_SANDBOX_MEMORY_MB`, default 256), result-size cap (default 1MB, kill-don't-buffer), crash containment (non-zero exit / spawn error / malformed output → honest `ok:false`, never a 500). Enabled via `PLUGIN_SANDBOX_MODE=off|process` (default off) + `PLUGIN_SANDBOX_PLUGINS=<id,id>|*` (default none) — NO SDK contract change (operator decides which plugins need isolation; least privilege by default). `PluginToolService` routes through it via a trailing `@Optional()` param — in-process dispatch byte-identical when off; the two-layer authz + audit/events unchanged. **11 new unit tests** (opt-in selection matrix; spawn seam — success, crash, timeout-kill, spawn-error, malformed, overflow, job-content).
  - **🐛 TWO REAL BUGS found LIVE (offline gates could not catch):** (1) the runner-path resolver picked a NON-EXISTENT path (`apps/api/scripts/…` — a broken resolve/endsWith check) so every child died `CJSWithHooks … MODULE_NOT_FOUND`; containment held (honest `ok:false`, api survived) but no tool ran. Fixed with exists-aware candidates + `PLUGIN_SANDBOX_RUNNER` override. (2) a "hang" fixture built on a bare `new Promise(() => {})` doesn't hang — the event loop drains and node exits itself (~200ms); a REAL hang needs an event-loop handle (a timer). Documented in the fixture + this log.
  - **LIVE PROOF (graphify + a TEMPORARY `sandbox-test` fixture plugin with ping/boom/crash/hang tools, 8s timeout; evidence in `artifacts/plugin-sandbox/`):** `graphify.graph.query` ran ENTIRELY in the child against the live MCP sidecar → `ok:true`, 39 real nodes with file:line provenance (a sandboxed tool does real work); `sandbox.ping` → `ok:true "sandbox-test says hi from the child process"`; `sandbox.boom` → contained `"plugin threw: boom from the sandbox"`; `sandbox.crash` (`process.exit(1)` in the child) → contained `"failed (exit 1)"` and **`/api/health` 200**; `sandbox.hang` → **killed at exactly 8000ms** (`"killed by timeout after 8000ms"`, durationMs 8014) and **`/api/health` 200**. The fixture plugin was deleted after the proof; the tree is fixture-free. Network isolation is NOT enforced on this host (Windows has no per-process network namespaces) — documented honestly; process/memory/time isolation is the v1 contract.
  - **Gates (`--force --concurrency=1`):** lint/build/typecheck/test **20/20**, 0 cached · tests **571 → 582** (api **447 → 458**: +11 sandbox). Tree clean after docs. Nothing pushed. $0/local unbroken.

- **2026-08-04 — ⚙️ PHASE 2.0 item 2.8 — WORKER AS A SEPARATE PROCESS: DONE, gate-verified, LIVE-PROVEN (git `3b37129`, local only). Polaris (solo round).** The BullMQ worker ran in-process with the api — a crashed worker took the api down with it. This round extracts it: the engine loops can run in a dedicated process against the same Redis queue, so the api stays responsive and workers scale horizontally.
  - **Design:** `ENGINE_WORKER_MODE=embedded` (DEFAULT — the api runs the loops in-process, byte-identical behavior) | `separate` (the api serves REST + enqueues ONLY; the AgentWorker, the scheduler poll and the supervisor sweep run in the dedicated process). `apps/api/src/worker-main.ts` — new standalone entrypoint (`NestFactory.createApplicationContext`, no HTTP server, a REF'D keep-alive interval because the loop timers are unref'd and would let the process exit). `engineLoopsRunHere()` (`engine-worker-role.ts`) is the single source of truth every loop service consults at `onModuleInit` (defer + honest WARN log) AND in its health shape — `/api/engine/health` honestly reports `scheduler.enabled:false` / `supervision.enabled:false` in the api process when separate. `scripts/boot-worker.sh` mirrors the api boot env (DB, Redis, model keys incl. DeepSeek, sandbox vars). **4 new role tests**; every existing engine suite untouched and green.
  - **LIVE PROOF (evidence in `artifacts/worker-separate-process/`):** (1) api with `ENGINE_WORKER_MODE=separate` → boot logs `AgentWorker NOT started here — deferred`, `SchedulerEngineService deferred`, `SupervisorService deferred`; health shows engine available but scheduler/supervision `enabled:false`; a submitted task stayed **`queued`** with NO consumer (the api does not consume in separate mode). (2) Booted the worker → it picked the queued task up and **completed it on Ollama** (`status:completed, provider:ollama`, cross-process queue handoff). (3) The worker's OWN scheduler poll fired a `* * * * *` cron schedule (`runCount 1`) — the runs-while-you-sleep loop lives in the worker. HA story: crash the worker, the api stays up; spawn more workers for horizontal scale. **Honest split-brain note:** the alerts ring buffer is per-process — in separate mode the api's `/api/engine/alerts` is empty (the worker's buffer is authoritative for its lifetime); documented, future work is a shared alert store.
  - **Gates (`--force --concurrency=1`):** lint/build/typecheck/test **20/20**, 0 cached · tests **582 → 586** (api **458 → 462**: +4 worker-role). Tree clean after docs. Nothing pushed. $0/local unbroken.

- **2026-08-04 — 🔑 PHASE 2.0 item 2.6 — REAL SSO ROUND-TRIP: DONE, gate-verified, LIVE-PROVEN (git `4d55928`, local only). Polaris (solo round).** The OIDC seam (P3/P4, `a07dd25`) was coded but never proven end-to-end against a running Keycloak. This round PROVED it: a real Keycloak-issued RS256 token is accepted by the api, the verifiers coexist, and the portal's federated tiles render through their SSO-proxied paths — all live, on the already-booted federation overlay.
  - **What shipped (tools only — no runtime code changes):** `scripts/boot-api-sso.sh` (api on :4001 with `OIDC_ISSUER_URL`/`OIDC_AUDIENCE` from root `.env` and `OIDC_JWKS_URI` overridden to the HOST-visible URL — the `.env` value `keycloak:8080` only resolves inside the compose network), `scripts/live-sso-proof.sh` (the four-curl acceptance), `scripts/live-sso-browser.sh` + `scripts/flow-sso-tools.json` (real-browser portal pass), `scripts/flow-login-debug2.json`.
  - **Realm state (was partially stale — the H2 in-memory realm is NOT persistent):** `constellation-portal` client + the `audience-constellation-portal` oidc-audience-mapper + `sso-user` (enabled, emailVerified) all existed; the user's password was reset via kcadm (`requiredActions=[]` + profile fields per the known trap). Real token decoded: `alg RS256` (asymmetric — not a local HS256), `iss http://localhost:8081/auth/realms/constellation` (matches `OIDC_ISSUER_URL`), `aud ["constellation-portal","account"]`, `sub fb1a110a-…` (Keycloak UUID).
  - **LIVE PROOF (literal evidence in `artifacts/sso-roundtrip/`):** (1) real Keycloak token → `GET /api/auth/me` **200** with `id:fb1a110a-c958-4cfc-bf68-235149634c9c` (the Keycloak principal — proves the OIDC path, not a local JWT); (2) local admin token → 200 `roles:["admin"]` (verifiers coexist — no regression); (3) tampered token (`${TOK}XX`) → **401** (signature check real); (4) no token → **401**. Boot log: `SSO enabled — OIDC verifier active: issuer="http://localhost:8081/auth/realms/constellation", audience="constellation-portal"`.
  - **Portal tile leg (REAL browser, zero-dep CDP, web dev :3005):** login → `/tools` renders the federated tile grid (Grafana under Observability; Open WebUI + Langflow under AI; Keycloak under Platform) with SSO/Embeddable tags, proxied paths (`/tools/grafana`, `/tools/chat`, `/tools/langflow`, `/auth`) and permission tags; screenshot clean. Caddy probes: `/auth/realms/constellation` 200 · `/tools/grafana/api/health` 200 · `/tools/prometheus/-/healthy` 200 · `/tools/grafana/` 302 (Grafana's own login redirect — documented behavior, not a proxy fault).
  - **Honest notes:** the realm lives in Keycloak's in-memory H2 (`start-dev`) — it does NOT survive `docker compose down`; the kcadm realm recipe in the federation reference is the reproducible path. Caddy's container healthcheck stays `unhealthy` while its `api` upstream container is down (env state — the api runs on the host for these proofs; the caddy-tile routes to grafana/prometheus/keycloak all work). One early browser pass timed out on the login redirect — root cause was the Next dev cold-compile hydration race (submit clicked before the bundle hydrated), fixed with a hydration sleep in the flow; not a product bug.
  - **Gates (`--force --concurrency=1`):** lint/build/typecheck/test **20/20**, 0 cached · tests **571** (api **447** · sdk 21 · graphify 40 · browser-use 47 · cli 16 — see the DeepSeek entry below for the +24). Tree clean after docs. Nothing pushed. $0/local unbroken.

- **2026-08-04 — 🚀 ENGINE — DEEPSEEK MODEL PROVIDER (direct API, like OpenRouter): DONE, gate-verified, LIVE-PROVEN (git `2d813de`, local only). Polaris (solo round).** The user provided a DeepSeek API key + the pricing page and asked for DeepSeek wired in like OpenRouter, using `deepseek-v4-flash`, to check product efficiency. DeepSeek is now a THIRD `ModelProvider` — the engine routes to it per task and falls back to Ollama exactly like OpenRouter.
  - **What shipped (code `2d813de`):** `DeepSeekModelProvider` (`apps/api/src/core/engine/deepseek-model-provider.ts`) — OpenAI-compatible `POST <base>/chat/completions` (base `https://api.deepseek.com`, Bearer), OPT-IN via `DEEPSEEK_API_KEY` (unconfigured → honest `health()` + `canHandleModel=false`, $0/local invariant), thinking-mode toggle (`{"thinking":{"type":"disabled"}}` default — the engine's JSON-action loop doesn't need CoT; `DEEPSEEK_THINKING=enabled` + `DEEPSEEK_REASONING_EFFORT` to switch), and **costUSD DERIVED from the documented list pricing** (DeepSeek sends no cost field): env-configurable rates defaulting to the v4-flash prices from the pricing page the user linked — input cache-hit $0.0028/M, input miss $0.14/M, output $0.28/M, with `prompt_cache_hit_tokens` credited at the hit rate. Registered in `engine.module.ts` (`MODEL_PROVIDERS = [ollama, openrouter, deepseek]`). `.env.example` OPT-IN section (placeholder only). Boot script exports the key + tuning vars ONLY when non-empty. **21 new provider unit tests** (headers/body, thinking toggle both ways, usage + derived cost incl. cache-hit credit + env rate overrides, 5xx-transient/401/404-terminal, network/timeout, unconfigured-safety, canHandleModel matrix).
  - **🐛 REAL BUG #1 found LIVE (offline gates could not catch):** the router's first-match scan handed the BARE cloud id `deepseek-v4-flash` to **Ollama** — Ollama's `canHandleModel` accepts every non-slash id and sits at `providers[0]`, so the task died `Ollama returned HTTP 404: model 'deepseek-v4-flash' not found`. The v0.3 routing only ever worked because OpenRouter ids always contain `/`. **Fix (`model-router.service.ts`): cloud (non-default) providers now scan BEFORE the default provider** — `deepseek-v4-flash` reaches DeepSeek, `qwen2.5-coder:7b` still lands on Ollama, slash ids on OpenRouter, no-match still falls back to Ollama's DEFAULT_MODEL (every v0.3-pinned behavior preserved; 2 new regression tests + `deepseek:` prefix stripping).
  - **🐛 REAL BUG #2 found LIVE (latent since the OTel round):** `TracingService.withSpan`'s disabled path ran the callback with `undefined` — the router's span callback dereferences it (`span.setAttributes`) on the first model call carrying usage, so **EVERY task failed** with `Cannot read properties of undefined (reading 'setAttributes')` whenever `OTEL_EXPORTER_OTLP_ENDPOINT` was unset (the default). The offline tests construct the router without a TracingService so they stayed green. **Fix: `withSpan` hands a shared NOOP_SPAN when disabled** ("spans are no-ops" now literally true); regression test added. A boot-script empty-env export (empty `DEEPSEEK_DEFAULT_MODEL` overriding the code default → `health()` reported `model:""`) was also fixed.
  - **LIVE PROOF (real DeepSeek API, key from root `.env`, evidence in `artifacts/deepseek-provider/`):** task `cmseljui00006s4fxq9mbar58` (`model: "deepseek-v4-flash"`) → router selected deepseek → **completed, `provider:"deepseek"` recorded honestly**, step `[0] done {result:"deepseek live-proof OK"}`. Health aggregate: ollama reachable, openrouter reachable, **deepseek reachable `model:deepseek-v4-flash`**. Direct probe (thinking disabled): the API ACCEPTS `{"thinking":{"type":"disabled"}}` live; usage `{prompt_tokens:9, completion_tokens:3, total_tokens:12, prompt_cache_hit_tokens:0, prompt_cache_miss_tokens:9}`; derived costUSD `2.1e-6`. **Total cloud spend this round ≈ $0.00001** (two task runs + one probe). The no-key fallback path is unchanged from v0.3 (proven then; unconfigured-safety unit-tested now).
  - **Honest note on the key:** the user pasted the DeepSeek key in chat — per the standing secrets discipline it was written ONLY to the git-ignored `.env` (verified `git check-ignore .env`), never to any tracked file, and never echoed. The chat copy is exposed; the user should rotate it on https://platform.deepseek.com/api_keys at their convenience. DeepSeek's announced peak/off-peak 2x pricing policy is not modeled (documented in the provider + `.env.example`).
  - **Gates (`--force --concurrency=1`):** lint/build/typecheck/test **20/20**, 0 cached · tests **547 → 571** (api **423 → 447**: +21 deepseek provider, +2 router regression, +1 tracing regression). Tree clean after docs. Nothing pushed. $0/local unbroken.

- **2026-08-04 — 🖥️ PHASE 2.0 item 2.4 — PORTAL `/health` ENGINE DASHBOARD: DONE, gate-verified, LIVE-PROVEN IN A REAL BROWSER (git `0647666`, local only). Polaris (solo round).** The engine's health was `curl | jq`-only; this round turns it into a visual, operator-facing page.
  - **What shipped (code `0647666`):** `apps/web/src/app/health/page.tsx` + `components/health/health-dashboard.tsx` (5s client poll of the public `GET /api/engine/health`; engine status hero with available/unavailable pill+reason, queue depth card incl. the v0.5 durable `failedTasks` count, model providers card rendering the aggregated `providers[]` (ollama + openrouter), scheduler card (poll loop/interval/last sweep/due count/event hooks), supervisor card (stale/recovered/stalled totals), alert-trail card with an honest "all quiet" empty state; DESIGN_SKILL language — `surface`/`surface-hover` cards, staggered `Reveal`, both themes, degraded states for API-unreachable/engine-unavailable). `lib/engine.ts` `EngineHealth` extended to the REAL v0.5 payload (the old client shape predated v0.4/v0.5 and would have dropped scheduler/supervision/alerts) + `EngineAlert` type. `lib/nav.ts` `core-health` item (Activity icon, order 50). `scripts/flow-health.json` reusable browser flow.
  - **LIVE-PROVEN (real Chrome via the zero-dep CDP driver, api :4001 + web dev :3005, admin login):** the dashboard rendered LIVE data — "Engine available" badge, queue Waiting 0 · Active 0 · Failed jobs 0 · Failed tasks 0, ollama (qwen2.5-coder:7b) **reachable** + openrouter cards, Scheduler "Poll loop running / 30s / last sweep 13s ago", Supervisor 0/0/0, alert trail "All quiet — no engine alerts recorded this process lifetime.", sidebar **Health** item highlighted, footer "Auto-refreshes every 5s · Last updated 0s ago". Screenshot + evidence: `artifacts/health-dashboard/`.
  - **Honest notes:** the page sits behind the portal session guard like `/engine` (unauthenticated browsers redirect to `/login?redirect=%2Fhealth` — the ENDPOINT is public); UI-only round, no test-count delta (web's gates are build+typecheck).
  - **Gates (`--force --concurrency=1`):** lint/build/typecheck/test **20/20**, 0 cached · tests **547** (unchanged — api 423 · sdk 21 · graphify 40 · browser-use 47 · cli 16). Tree clean after docs. Nothing pushed. $0/local unbroken.
...
- **2026-08-03 — 🛡️ PLATFORM HARDENING (v0.6): DONE, gate-verified, LIVE-PROVEN, COMMITTED (git `070fb2d`, local only) — Polaris orchestrating via delegate_task.** Three non-engine platform items:
  ...
  - **Per-plugin schema bootstrap (C8)** — `PrismaService.bootstrapSchema(schema)` issues `CREATE SCHEMA IF NOT EXISTS` (with a defensive identifier+injection check) + graceful no-DB degrade; `PluginContextFactory.schemaBootstrap(manifest)` wired into `plugin-loader.loadOne` + `plugin-lifecycle.enable`, per-process `Set` cache.
  - **httpOnly-cookie auth** — new `auth-cookie.ts` (set/clear/read; `httpOnly; SameSite=Lax`, `Secure` in prod, `AUTH_COOKIE_NAME`); login sets the cookie (token still in body for bearer clients), logout clears it; `JwtAuthGuard` reads the cookie when no bearer header (bearer flow untouched); portal sends `credentials:include` + restores a cookie-only session. CSRF caveat documented (Lax; async double-submit deferred).
  - **Live-proven (postgres/redis:6380, api:4001):** (1) `/login` sets `Set-Cookie: constellation_token=...; Path=/; HttpOnly; SameSite=lax`; (2) cookie-only `GET /api/auth/me` returns the admin principal (no bearer); (3) `viewer@constellation.local` login → `roles:[viewer], permissions:[core:authenticated]` ONLY, and viewer → `GET /api/audit` → **403** while viewer → `GET /api/engine/schedules` → 200 (no over-blocking). Evidence: `artifacts/platform-hardening-v0.6/`.
  - **Gates (`--force --concurrency=1`):** 20/20 tasks · api **402** (repo **519**, up from 505, +14 from `auth-cookie.test.ts`). Web typecheck + build green. Tree clean after docs. Nothing pushed.

- **2026-08-03 — 🤖 ENGINE v0.5 — "DEEPER 24/7 RELIABILITY": DONE, gate-verified, LIVE-PROVEN, COMMITTED (git `ec88534`, local only) — Polaris orchestrating via delegate_task (maker subagent built the code; Polaris fixed the boot-DI bug, wrote the missing supervisor/alerts/TaskService/controller unit tests, ran the gates, and LIVE-PROVED dead-letter + supervisor + alerting end-to-end). ROUND SUMMARY:** The engine ran tasks durably but had NO structured dead-letter trail, NO detection of stuck (`running` forever) tasks, and NO alerting surface. This round adds all three so real 24/7 operation is observable and recoverable. **This is roadmap item ORCHESTRATOR §3 #3.**
  - **What shipped (code `ec88534`):**
    - **`dead-letter.ts`** — `FailureClassification` (`terminal | transient_exhausted | stalled | rejected`), `DEAD_LETTER_CLASSIFICATIONS`, `DEAD_LETTER_LIMIT`, `DeadLetterTask`.
    - **`TaskService` v0.5** — `markFailed(id,error,classification?)` records the classification; `findAllFailed(limit)` (newest-first DLQ view), `getFailedCount()` (durable failed-task count, distinct from BullMQ's failed-job count); supervisor queries `findStaleRunning`, `markResumed`, `markStallRetried`.
    - **`SupervisorService`** — poll loop (`ENGINE_SUPERVISOR_INTERVAL_MS` default 30s, `ENGINE_STALE_TASK_MS` default 5min, unref'd timer, injectable clock/options, overlap guard). Detects tasks stuck `running` with no progress past the threshold. **RACE GUARD**: a stale task whose BullMQ job is ACTIVE is skipped, never double-run. **Resume-once**: a genuinely-stalled task is re-enqueued (`markResumed` + `markStallRetried`); a re-stale (already-resumed) task is marked `failed` with a `stalled` classification + honest error (no infinite spin). Availability-gated (`ran:false` when disabled). `getHealth()` block in `/api/engine/health`.
    - **`EngineAlertService`** — emits `engine.task.failed` / `engine.task.stale` / `engine.task.recovered` on the core EventBus scope (`@Optional()` + `try/catch`, NEVER throws) plus a capped in-memory ring buffer surfaced via new `GET /api/engine/alerts` and inside `/api/engine/health`. `@Optional() @Inject(ENGINE_ALERTS_CAP)` keeps the optional cap from breaking Nest DI at boot.
    - **Controller** — new `GET /api/engine/deadletters` (authed) + `GET /api/engine/alerts`; `health()` now carries `failedTasks`, `supervision`, `alerts`. AgentWorker failure paths classified; reject -> `rejected`.
  - **Tests:** api **364 -> 388** (repo **481 -> 505**): `supervisor.service.test.ts` (race guard, resume-once, re-stale->stalled, disabled no-op, health counters), `engine-alerts.service.test.ts` (ring buffer + cap + emit-on-bus + absent/throwing bus safety), `task.service.v05.test.ts` (classification, DLQ view/count, supervisor queries, no-DB degradation), `engine.controller.test.ts` (deadletters + alerts endpoints), plus updated agent-worker/engine-controller tests for the new signatures.
  - **🐛 Real boot bug the offline tests missed (found on live boot):** `Nest can't resolve dependencies of the EngineAlertService (EventBusService, ?).` — the optional `cap` constructor param had no DI token, same class as the v0.4 options param. Fixed with `@Optional() @Inject(ENGINE_ALERTS_CAP)`.
  - **LIVE PROOF (postgres:5432, redis:6380, ollama `qwen2.5-coder:7b`, api:4001, supervisor poll 5s, stale 25s):** (1) **Supervisor recovered a stale task**: a `running` task updated 122s ago (no active job) was flagged (`engine.task.stale`, detail `122239ms`) and re-enqueued (`engine.task.recovered`), then the live worker genuinely completed it (`stepCount:1`) — proving recovery + the race guard never double-ran it. (2) **Stalled dead letter**: a re-stale already-resumed task became `status=failed`, `failureClassification=stalled`, error `"stalled after resume attempt -- no progress for 122s"` — the no-infinite-spin guarantee. (3) **`GET /api/engine/deadletters`** returned the classified row; `/api/engine/alerts` showed the full `failed → recovered → stale` trail; health showed `failedTasks:1`, `supervision {staleFound:4, recovered:1, failedStalled:1}`, `alerts:3`. Literal evidence: `artifacts/engine-v0.5-reliability/live-proof-evidence.md`.
  - **Final gates (`--force --concurrency=1`):** lint/build/typecheck/test **20/20** · tests **505** (api **388** · sdk 21 · browser-use 47 · graphify 40 · cli 9). Tree clean. Nothing pushed. $0/local unbroken.

- **2026-08-03 — 🤖 ENGINE v0.4 — "SCHEDULER / AUTONOMOUS TRIGGERS": DONE, gate-verified, LIVE-PROVEN, COMMITTED (git `f70b573`, local only) — Polaris orchestrating via delegate_task (maker subagent built the code; Polaris wrote the missing service/engine/controller unit tests, ran the full mono gates, and LIVE-PROVED the scheduler end-to-end). ROUND SUMMARY:** The engine until now only ran tasks when a human POSTed to `/api/engine/tasks`. This round adds recurring (cron) and event-triggered task schedules that AUTO-ENQUEUE tasks — the "runs while you sleep" capability. **This is the top roadmap item (ORCHESTRATOR §3 #2).**
  - **What shipped (code `f70b573`):**
    - **`ScheduledTask` Prisma model** (`schema.prisma`, `core` schema -> `scheduled_tasks`): id, task template (title/prompt/model/maxSteps/maxTokens), `kind` (cron|event), flexible `spec Json` (cron expr + timezone / event topic), enabled, nextRunAt/lastRunAt/runCount/lastError, indexes on `[enabled,kind]` / `[enabled,nextRunAt]`. Additive.
    - **`ScheduledTaskService`** — CRUD + `listDueCronSchedules`/`listEnabledEventSchedules`/`markRun` (advances nextRunAt via the cron parser + records lastError), exact `this.prisma.db` no-DB degradation (reads -> []/null; writes throw "Database not available"). Injectable `now` clock.
    - **`cron.ts`** — **zero-dep hand-rolled 5-field crontab parser** (`*`, values, ranges `a-b`, lists `a,b,c`, steps `*/n`/`a-b/n`, dow 0/7 -> Sunday, AND dom/dow documented), `CronParseError`, `nextRunAfter`, `validateCron`, `secondsUntilNext`. No new npm deps (matches the codebase's zero-dep `fetch`/`AbortSignal` pattern).
    - **`SchedulerEngineService`** — poll loop every `SCHEDULER_POLL_INTERVAL_MS` (default 30s, timer unref'd, overlapping-sweep guard); lists due ENABLED cron schedules, enqueues a task each (reusing `TaskService.create` + `TaskQueueService.enqueue` with the schedule's template, `actorId:null` = system-authored), advances run bookkeeping. Kind "event" schedules are skipped by the clock and fired by a deduped EventBus listener (via `@Optional()` EventBusService; handler re-fetches from DB so disable/delete naturally stops firing). Honest disabled-engine degradation (`ran:false`, no churn); per-schedule failure -> `markRun(err)` records lastError + advances nextRunAt (no spin). Public `runSweep(now?)`/`fireSchedule()` seams + injected `now`/`pollIntervalMs` for offline tests. `getHealth()` (enabled/pollIntervalMs/lastSweepAt/dueCount/registeredEvents) joins `/api/engine/health`.
    - **`SchedulerController`** — `POST/GET /engine/schedules`, `GET /:id`, `POST /:id/enable`, `POST /:id/disable`, `DELETE /:id`; auth-guarded, mutations audited (`engine.schedule.created/enabled/disabled/deleted`), `CronParseError` -> 400.
    - **`.env.example` + boot script** — `SCHEDULER_POLL_INTERVAL_MS` documented; `scripts/boot-api-v0.3.sh` + new `scripts/boot-scheduler-live.sh` (5s poll for fast live proofs).
  - **Tests:** api **295 -> 364** (repo **412 -> 481**; +69): `cron.test.ts` (28: valid/invalid expressions, nextRunAfter incl. day-of-week/month/day/steps, never-before-anchor), `scheduled-task.service.test.ts` (CRUD + no-DB degradation + cron validation + markRun advance), `scheduler-engine.service.test.ts` (sweep fires due once + advances, error counting + lastError, disabled -> ran:false, overlapping guard, getHealth, event listener fires + skips clock), `scheduler.controller.test.ts` (create/audit/refreshListeners, CronParseError->400, enable/disable/delete + 404s), plus engine.controller.test.ts updated for the new scheduler health dep.
  - **🐛 Real boot bug the offline tests could NOT catch (found on live boot):** `Nest can't resolve dependencies of the ScheduledTaskService (…, ?).` — the services' optional constructor `options` param had no DI token, so the container failed to resolve it at boot. Fixed by marking it `@Optional() @Inject(SCHEDULER_SERVICE_OPTIONS)` / `SCHEDULER_ENGINE_OPTIONS` (unregistered token -> resolves `undefined` in prod; offline tests still pass options directly via `new`). This is the exact "green gates while the real app breaks" class the operating method warns about — the live boot caught it.
  - **LIVE PROOF (real local infra: postgres:5432, redis:6380, ollama `qwen2.5-coder:7b`, api:4001, poll 5s):** `GET /api/engine/health` -> `scheduler:{enabled:true, pollIntervalMs:5000}`. `POST /api/engine/schedules {"name":"polaris-live-proof","kind":"cron","cron":"* * * * *","task":{title,prompt,maxSteps}}` -> `nextRunAt:…T17:08:00Z` (cron parser computed it). At the boundary the poll loop **auto-enqueued a task** — `cmsdhg4rz…`, `title:"scheduled digest"`, `actorId:null` (system-authored) — which **COMPLETED on Ollama**: `status:"completed"`, `stepCount:1`, `provider:"ollama"`, `steps:[0] done {result:"scheduler live-proof OK"}`. Schedule `runCount` advanced 0->1->2 (fired again at the next minute, autonomous recurrence), `nextRunAt` kept advancing, `lastSweepAt` refreshed every 5s. `DELETE /schedules/:id` -> `{deleted:true}` + subsequent GET -> 404 (no further firing). Literal evidence: `artifacts/engine-v0.4-scheduler/live-proof-evidence.md`.
  - **Final gates (`--force --concurrency=1`):** lint/build/typecheck/test **20/20** · tests **481** (api **364** · sdk 21 · browser-use 47 · graphify 40 · cli 9). Tree clean. Nothing pushed. $0/local unbroken (the proof ran on local Ollama; OPENROUTER key unset -> honest `reachable:false`).

- **2026-08-03 — 🤖 ENGINE v0.3 — "REAL MODEL PROVIDERS": ALL 8 TASKS DONE, gate-verified, LIVE-PROVEN, COMMITTED (git `3d7d635` → round summary commit, local only) — clau_partner (orchestrating solo, Nova/Orion/Atlas resting). ROUND SUMMARY:**
  - **What the round was:** the engine ran ONLY on local Ollama. This round added a SECOND `ModelProvider` — OpenRouter (one OpenAI-compatible key unlocks GPT-OSS/Qwen/DeepSeek/Claude/…), upgraded `ModelRouterService` from "first-provider-wins" to REAL routing + fallback, made the budget seam COST-AWARE (`ModelUsage.costUSD`), and proved it live BOTH ways: a real cloud task completing on real tool data, and the $0/no-key invariant holding with nothing crashing.
  - **Task 1 — `OpenRouterModelProvider` (`3d7d635`):** OpenAI-compatible `POST <base>/chat/completions` (Bearer auth, `HTTP-Referer`/`X-Title`), usage + `costUSD` parsing (`usage.cost` or per-token pricing), 5xx/network/timeout transient · 4xx (incl. 401/403 bad key) terminal, never-throwing `health()` (honest `OPENROUTER_API_KEY is not set`), `canHandleModel()` (slash ids + `openrouter:` prefix; false when unkeyed). Interface: `ModelUsage.costUSD?` + optional `ModelProvider.canHandleModel?`.
  - **Task 2 — provider unit tests (`341f30d`):** 22 tests, mocked fetch, no network (headers/body, usage+cost, error classes, timeout abort, health matrix, unconfigured safety).
  - **Task 3 — real routing + fallback (`9cd9ce4` + `aa18f7b`):** `selectProvider` by `canHandleModel` (no model → Ollama default); ANY non-default failure → **Ollama with DEFAULT_MODEL** (logged); `openrouter:`/`ollama:` prefixes stripped; health() aggregates all providers. Follow-up `aa18f7b`: a NO-MATCH model (e.g. "/" id while OpenRouter unconfigured) also falls back to Ollama's DEFAULT_MODEL — the raw id would 404 terminally (discovered while preparing Task 8; its unit test updated).
  - **Task 4 — router tests (`e97c290`):** 12 new (routing, prefix stripping, fallback transient+terminal, Ollama-failure propagates, aggregated health ×3); all 14 existing kept verbatim.
  - **Task 5 — wiring (`28c56b0`):** EngineModule registers OpenRouter (`MODEL_PROVIDERS=[ollama, openrouter]`); the hardcoded `markRunning(taskId,"ollama")` is GONE — the task is marked running without a provider and `TaskService.markProvider(taskId, response.provider)` records the ACTUAL provider after the first model call (no dishonest state; 2 new worker tests).
  - **Task 6 — `.env.example` (`fe4b215`):** OPT-IN OpenRouter section, placeholder key only, never a real key; per-task `"model"` unlock docs.
  - **Task 7 — cloud E2E LIVE-PROVEN (`1054e90`):** task `cmsdegbhl000648fx5bfaz0up` (`model:"openai/gpt-oss-120b"`) → router selected OpenRouter → CLOUD model drove `[0] tool_call graphify.graph.query` → `[1] tool_result ok:true` (48 real nodes, file:line provenance) → `[2] done` (grounded summary) → **completed, `provider:"openrouter"` honestly recorded**. Usage+cost live: direct probe `usage:{72,36,108, cost 8.28e-6}` — parsed into `costUSD` (unit-pinned). **Total cloud spend ≈ $0.00001.** Honest gap recorded: per-call usage/cost is not persisted on the task record (in-memory `TokenBudget` only) — follow-up noted.
  - **Task 8 — no-key fallback LIVE-PROVEN (`a32f6e7`):** key ABSENT → boot clean, health honest (`openrouter reachable:false "OPENROUTER_API_KEY is not set"`), three tasks (no-model / `openai/gpt-oss-120b` / `qwen2.5-coder:7b`) ALL **completed on Ollama** with the honest provider field — the "/" task via the no-match DEFAULT_MODEL fallback (literal log: `Model router: no provider can handle "openai/gpt-oss-120b" — falling back to Ollama (ollama) with its default model`).
  - **Final gates (`--force --concurrency=1`):** lint **20/20** · build **7/7** · typecheck **8/8** · test **412** (api **295** · sdk 21 · browser-use 47 · graphify 40 · cli 9) — up from 376 at round start (**+36**).
  - **The $0/local invariant, live-proven:** Ollama remains the default; OpenRouter is opt-in per task; no key → nothing crashes, health honest, tasks complete on Ollama. Nothing pushed. Tree clean at round end. Full literal evidence in the per-task entries above + `artifacts/engine-v0.3/`.

- **2026-08-03 — 🤖 ENGINE v0.3 — REAL MODEL PROVIDERS, TASK 7 of 8: LIVE-PROVEN cloud model end-to-end — clau_partner (orchestrating solo).**
  - **What was proven (full stack live, key set):** task `cmsdegbhl000648fx5bfaz0up` with `model:"openai/gpt-oss-120b"` → router selected OpenRouter (slash id + key set) → the CLOUD model drove the loop: `[0] tool_call graphify.graph.query {"question":"What services does PluginLifecycleService depend on?"}` → `[1] tool_result ok:true` (real graph traversal, BFS depth=3, 48 nodes, file:line provenance — literal in the record) → `[2] done` → status **`completed`** and the task's provider field honestly recorded as **`openrouter`** (the v0.3 markProvider — not a hardcoded value). The done summary is grounded in the tool data: *"PluginLifecycleService depends on core services such as PrismaService, PluginRegistryService, EventBusService, and PluginContextFactory, and also relies on external packages like commander and @constellation/plugin-sdk."*
  - **Usage + cost (acceptance e/f):** OpenRouter's literal response carries tokens + USD cost — a direct live probe against the same model returned `usage:{prompt_tokens:72, completion_tokens:36, total_tokens:108, cost:8.28e-6}` (saved `artifacts/engine-v0.3/or-probe-usage-cost.json`; the key was never echoed). The provider parses this into `usage.inputTokens/outputTokens/totalTokens/costUSD` — pinned by the 22 provider unit tests. **HONEST GAP (recorded, not hidden):** per-call usage/cost is NOT persisted on the task record — it feeds the in-memory `TokenBudget` only (`agentTask` has no usage column; steps carry tool results). Follow-up: persist per-call usage as a step/column (cheap).
  - **Cost reality:** the entire cloud proof cost ≈ **$0.00001** — the round stays effectively $0.
  - **Evidence committed:** `artifacts/engine-v0.3/task7-cloud-e2e.json` + `or-probe-usage-cost.json` (code `1054e90`); full four-gate pass at round end (**412 tests**).
