...
> **Status:** Platform layer strong + **the agentic engine is real and live-proven** through **Engine v0.3** (git `c63f6bd` → round-end summary SHA, local only — never pushed). Durable BullMQ task runtime, ReAct loop with checkpoint/resume (kill-restart proven live, incl. across a real tool call), **human-in-the-loop approval gate** (pause→approve/reject, honour-once, audited), per-task token budget cap, `/engine` portal (browser-verified) — plus **Engine v0.3 — REAL MODEL PROVIDERS**: OpenRouter as a SECOND `ModelProvider` (one key → GPT-OSS/Qwen/DeepSeek/Claude/…), real routing + fallback in `ModelRouterService` (`canHandleModel` selection; non-default failure OR no-match → Ollama with DEFAULT_MODEL; `openrouter:`/`ollama:` prefix stripping; aggregated health), and a **cost-aware budget** (`ModelUsage.costUSD` — OpenRouter's `usage.cost` flows through). **PROVEN LIVE both ways:** a cloud task (`openai/gpt-oss-120b`) completed on real tool data with the provider honestly recorded (`provider:"openrouter"`, task `cmsdegbh…`, ~$0.00001); and with NO key the engine stays $0/local — every task completes on Ollama, health honest, nothing crashes. **412 tests green** (api 295, browser-use 47, graphify 40, sdk 21, cli 9). See §9. **Next: a scheduler.** VPS deferred (prove locally first).
> **Relationship to Looper:** SEPARATE project. Looper (`../loop-engineering`) is untouched.
> **Last updated:** 2026-08-03 (clau_partner — Engine v0.3 round: OpenRouter provider + real routing/fallback + cost-aware budget, live-proven both ways, 412 tests)
