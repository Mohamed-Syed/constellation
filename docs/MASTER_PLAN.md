...
| **Multi-model compare (3.6)** | Polaris (solo) | Usage/cost PERSISTED on tasks (TokenBudget input/output/cost accumulation, worker persistUsage at every terminal path, migration add_task_usage) + portal /compare (same prompt on 2+ models, side-by-side latency/tokens/$ cards + summary table); nav entry | `f2afda4` | `docs-commit-below` | 513 → **516** (+3) | ✅ LIVE: ollama 51.0s/$0 vs deepseek-v4-flash 2.3s/$0.000018, persisted tokens both; browser results table + cards verified |
| **Notification channels (3.5 remainder)** | Polaris (solo) | Webhook channels (generic/Slack/Discord/Teams envelopes, per-kind filters, enabled toggle, fire-and-forget fetch+timeout), REST /notifications/channels + Test, NEW engine.task.completed/paused events, portal admin Channels tab | `88944b7` | `docs-commit-below` | 516 → **532** (+16) | ✅ LIVE: local listener — generic got completed+failed, slack only the failure, Test + browser Test delivered POSTs |
| **Workflow trigger wiring** | Polaris (solo) | ScheduledTask.workflowId (migration add_workflow_triggers) → scheduler RUNS workflows (fire-and-forget); WorkflowTriggerService cron auto-schedules + event listeners on core+platform scopes; bidirectional forwardRef; = autonomous incident-response primitive; 1 real test-found bug (deactivate-all-keys on trigger change) | `ee5b304` (+ fixup `4b426f8`) | `docs-commit-below` | 532 → **541** (+9) | ✅ LIVE: cron workflow auto-armed → fired → completed; PUT re-sync; doomed task fired engine.task.failed event workflow → completed |
| **Team spaces API (3.7, slice 1)** | Polaris (solo) | Organization/Team/TeamMember (owner\|admin\|member, migration add_team_spaces) + /api/teams RBAC (403 on non-manager), /me teams, AgentTask.teamId scoping (submit-into-team membership check; non-admin union list); portal /teams page + live proof IN FLIGHT | `418c0d8` | `docs-commit-below` | 541 → **554** (+13) | ✅ 38 files green; browser proof pending (next round) |
