...
| 3.5 | ✅ **Notification center** — **SHIPPED 2026-08-04 (round 3.4, §9):** durable in-portal event feed (engine alerts + scheduler events), REST API, sidebar unread badge, admin audit-log tab. **Channels SHIPPED 2026-08-05 (`88944b7`):** webhook channels (generic/Slack/Discord/Teams envelopes, per-kind filters, Test) + engine.task.completed/paused events. Remaining (documented follow-ups): email/SMTP channel + per-task user-configurable notification. | "Runs while you sleep" — but you need to know when it's done. Every enterprise platform has this. | Grafana alerting, PagerDuty, every CI system | P1 |
...
| 3.7 | 🚧 **Team spaces / multi-tenancy** — **API SHIPPED 2026-08-05 (`418c0d8`, §9):** `Organization → Team → User` + TeamMember roles (owner/admin/member), inherited RBAC (member management owner/admin-only), `/api/teams` REST, `/me` carries teams, `AgentTask.teamId` scoping (submit-into-team requires membership; non-admins see personal + their teams' tasks). **In round: portal /teams page + live browser proof.** Follow-ups: team-scoped schedules/workflows/plugins, org roles, invitations. Team A can only see/edit their own tasks — admin sees everything. | The #1 request from any org with >5 users. | Every enterprise platform (Backstage RBAC, Port teams, GitHub orgs) | P2 |
