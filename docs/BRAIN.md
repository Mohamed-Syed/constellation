...
| **PAUL + SEED** | Skip (for now) | A Claude Code *build* methodology, not runtime memory. We already run spec→build→verify via MASTER_PLAN + managed subagents. Don't conflate build tool with brain. |
...
## 6. Build split (lanes)
- **Atlas (infra):** add a `graphify` service to `docker-compose.yml` (python image, `pip install
...
- **Nova (core/agent):** `apps/api/src/core/memory` `BrainService` + `GraphifyAdapter` + REST
...
- **Orion (portal):** a "Brain" nav item + page — render `graph.json` (force-directed) and an
