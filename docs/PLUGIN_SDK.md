...
> For the platform's overall architecture and roadmap, see [ARCHITECTURE.md](ARCHITECTURE.md)
> and [ROADMAP.md](ROADMAP.md). This guide only covers the plugin authoring surface.
...
| Field | Type | Required | Notes |
|---|---|---|---|
| `databaseSchema` | string | no (defaults to `id`) | The Postgres schema this plugin owns. No plugin may read/write another's schema — see [ARCHITECTURE.md](ARCHITECTURE.md) (decision C8). |
...
contract), but several backends are foundation-phase stand-ins pending the core services
...
- No admin API to call `install`/`uninstall`/`disable` — those hooks exist in the contract but
  have no caller yet.
- `PluginContext.config`, `.events`, and `.db` are stubs (§6) — the settings/events/database
...
All of the above are on the platform roadmap (see [ROADMAP.md](ROADMAP.md)) — this guide will be
